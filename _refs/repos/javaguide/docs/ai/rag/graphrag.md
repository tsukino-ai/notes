...
RAG（Retrieval-Augmented Generation，检索增强生成）是一种把**信息检索（Information Retrieval，IR）**和**生成式大语言模型（LLM）**结合起来的框架。
...
GraphRAG 把检索对象从文本 Chunk 扩展到了实体、关系、路径、社区摘要。它适合多跳推理、影响分析、归因分析和复杂业务问答，但代价是数据治理成本更高。Neo4j GraphRAG 适合已有业务关系的场景；LangChain/LlamaIndex 等适合现有技术栈集成。选择哪条路线，要看你的技术栈、图模型复杂度和运维能力。
