...
把这些规则固化为 `.cursorrules` 或 `AGENTS.md` 文件，Agent 在复杂任务里的“脱轨”概率会大幅降低。随着模型能力提升，Prompt 格式的精确性可能没以前那么敏感，但结构化编排带来的**可维护性**和**团队协作效率**仍然很有价值。
...
> **工程提示**：Anthropic 的做法是"Calibrating the system prompt"——把 System Prompt 当成一个需要持续调校的参数，而不是一次性写死的产品配置文档。每发现一个 failure case，针对性地加一条清晰规则，然后重新测试。
