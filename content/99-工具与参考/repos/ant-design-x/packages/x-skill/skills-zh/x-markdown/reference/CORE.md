...
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills-zh/x-markdown/reference/CORE.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
| 管理 Provider、请求与消息状�?| `@ant-design/x-sdk`      |
...
- 先让�?Markdown 渲染正确，再加插件或自定义组件�?
- 当模型输出可能包含外链时，优先开�?`openLinksInNewTab`�?
- 如果原始 HTML 只需要保留文本展示，优先使用 `escapeRawHtml`�?
- 如果必须渲染真实 HTML，请显式检�?`dompurifyConfig`�?
...
�?`useXChat` 产出消息列表之后，再�?`XMarkdown` 负责内容展示�?
...
- 内容是流式分块到达时，阅�?[STREAMING.md](STREAMING.md)
- 需要自定义标签、代码块、插件、主题时，阅�?[EXTENSIONS.md](EXTENSIONS.md)
- 需要完整属性表时，阅读 `API.md`
