...
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills-zh/x-card/reference/API.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
`@ant-design/x-card` 的完�?TypeScript 类型�?
...
interface BaseComponent_v0_9 {
  ...
  component: string;
  child?: string;
  children?: string[]; // 子组�?ID 数组；模板迭代用 { path, componentId } 对象（运行时支持，类型层未体现）
...
interface ActionPayload {
  ...
  surfaceId: string;
  /**
   * 触发 Action 时当�?Surface 的完�?dataModel 快照�?
   * 注意：这是整个数据模型，不只�?action.event.context 中定义的字段�?
...
interface ComponentWrapper_v0_8 {
  ...
  component: {
    [componentType: string]: {
      ...
      [key: string]: any; // 支持 { path } �?{ literalString } 绑定
    };
  };
}

// v0.8 action 格式：context 是数组，�?v0.9 的对象格式不�?
interface ActionConfig_v0_8 {
  name: string;
  context?: Array<{
    key: string;
    value: PathValue | LiteralStringValue | any; // { path } 是写入目�?
  }>;
}
...
interface DataModelUpdate_v0_8 {
  ...
  contents: Array<{
    key: string;
    valueString?: string; // 直接存储字符串�?
    valueMap?: Array<{ key: string; valueString: string }>; // 转换为对�?
  }>;
}

interface BeginRendering_v0_8 {
  surfaceId: string;
  root: string; // 根组�?ID，v0.8 须显式指�?
}
