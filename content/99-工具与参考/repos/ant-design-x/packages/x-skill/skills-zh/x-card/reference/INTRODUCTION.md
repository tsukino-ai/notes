...
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills-zh/x-card/reference/INTRODUCTION.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
`@ant-design/x-card` 是一个基�?[A2UI 协议](https://a2ui.org/concepts/data-flow/) 的动态卡片渲染组件，�?AI Agent 能够通过结构化的 JSON 消息流，动态构建和渲染交互式界面�?
...
1. **流式消息（Streaming Messages�?*：UI 更新�?JSON 消息序列的形式从 Agent 流向客户�?
2. **声明式组件（Declarative Components�?*：UI 以数据形式描述，而非代码编程
3. **数据绑定（Data Binding�?*：UI 结构与应用状态分离，实现响应式更�?
...
| 特�?| A2UI | AI 生成 HTML |
| --- | --- | --- |
| **安全�?* | 仅使用预定义组件目录（Catalog），无代码执行风�?| 可能包含恶意脚本，存在注入风�?|
| **跨平�?* | 一套数据结构自动适配 Web、移动端等多端原生组�?| HTML 需额外适配不同设备，易出现样式错乱 |
...
`@ant-design/x-card` 同时支持 A2UI 协议�?v0.8 �?v0.9 两个版本。了解两个版本的差异有助于选择合适的协议版本并进行迁移�?

### 版本对比

| 特�?            | v0.8                                       | v0.9                          |
| ---------------- | ------------------------------------------ | ----------------------------- |
| **版本标识**     | 无显�?version 字段                        | 显式�?`version: 'v0.9'` 字段 |
| **Surface 创建** | 隐式创建（首�?updateComponents 自动创建�?| 显式 `createSurface` 命令     |
| **数据模型更新** | 使用 `contents` 数组                       | 使用 `path` �?`value` 字段   |
| **组件定义**     | 较复杂的嵌套结构                           | 更简洁的扁平结构              |
| **推荐程度**     | 已弃用，仅用于兼�?                        | **推荐使用**                  |

### v0.8 消息格式（已弃用�?

v0.8 使用隐式�?Surface 创建方式，当 Agent 发送第一�?`updateComponents` 时自动创�?Surface�?

```typescript
// v0.8 没有显式�?version 字段
{
  updateComponents: {
    surfaceId: 'booking',
...
如果您正在使�?v0.8，建议按以下步骤迁移�?v0.9�?

#### 1. 添加版本标识

所有消息添�?`version: 'v0.9'` 字段�?
...
在发�?`updateComponents` 之前，先发�?`createSurface`�?
...
组件会根据是否存�?`version` 字段自动判断协议版本，并正确处理消息�?

## 核心消息类型

`@ant-design/x-card` 完整实现�?A2UI 协议 v0.9 的核心命令系统：

### 1. createSurface - 创建界面容器

创建一个新�?UI 容器（Surface），每个 Surface 拥有独立的组件树和数据模型�?
...
定义或更�?Surface 中的 UI 组件，采用邻接表模型（Adjacency List）�?
...
A2UI �?UI 结构与应用状态分离，通过数据绑定实现响应式更新�?

### 数据模型（Data Model�?

每个 Surface 拥有独立�?JSON 数据模型�?
...
使用 RFC 6901 标准�?JSON Pointer 访问数据�?
...
// 路径绑定（动态）
{
  id: 'username',
  component: 'Text',
  text: { path: '/user/name' }  // 从数据模型读�?
}
```

�?`/user/name` �?`"Alice"` 变为 `"Bob"` 时，文本自动更新�?
...
```typescript
{
  id: 'product-name',
  component: 'Text',
  text: { path: 'name' }  // 相对�?/products/0�?products/1 �?
}
```

对于 `/products/0`，`name` 解析�?`/products/0/name` �?`"Widget"`
...
1. 更新 UI：发�?`updateComponents` �?`updateDataModel`
2. 关闭界面：发�?`deleteSurface`
3. 创建新界面：发送新�?`createSurface`

## 组件目录（Catalog�?

Catalog 定义了可用的组件及其属�?schema，确保类型安全和验证�?
...
Agent 和客户端通过协商确定使用�?Catalog�?

1. **客户端声明能�?*：发送支持的 `supportedCatalogIds`
2. **Agent 选择 Catalog**：从客户端列表中选择合适的 Catalog
3. **创建 Surface**：在 `createSurface` 中指�?`catalogId`
...
- **数据可视�?*：动态生成图表、列表等数据展示组件
- **工作流编�?*：根据业务流程动态渲染不同阶段的 UI
- **多轮对话**：在聊天界面中嵌入动态交互组�?
- **个性化界面**：根据用户偏好和使用场景定制 UI

## 性能优化建议

1. **细粒度更�?*：只更新变化的数据路径，而非整个数据模型
...
- 浏览 [组件结构](https://a2ui.org/concepts/component-structure/) 学习邻接表模�?
- 参�?[数据绑定](https://a2ui.org/concepts/data-binding/) 掌握响应式更�?
