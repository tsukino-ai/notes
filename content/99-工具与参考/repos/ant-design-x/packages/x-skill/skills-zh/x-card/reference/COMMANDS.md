---
title: A2UI 命令参考（v0.9�?
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills-zh/x-card/reference/COMMANDS.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
本文档涵�?`XAgentCommand_v0_9` 的四�?Server→Client 命令。需要了解命令结构或构建命令序列时阅读本文档�?
...
初始化新 UI Surface。必须在针对�?`surfaceId` 的任�?`updateComponents` �?`updateDataModel` 之前发送�?

```typescript
interface CreateSurfacePayload {
  surfaceId: string; // 唯一�?Surface 标识�?
  catalogId: string; // Catalog URI �?local:// 标识�?
  // 注意：theme �?sendDataModel �?A2UI 协议规范字段，当前实现尚未支持，传入会被静默忽略
}
```
...
interface BaseComponent_v0_9 {
  id: string; // Surface 内唯一的组�?ID
  component: string; // 组件类型名称（必须在 Catalog 中）
  child?: string; // 单个子组�?ID
  children?: string[]; // 多个子组�?ID；模板迭代时�?{ path, componentId } 对象（运行时支持�?
  [key: string]: any | { path: string }; // 所�?prop 均支持数据绑�?
}

// List 组件的模板模式（运行时支持，TypeScript 类型�?children 声明�?string[]�?
...
通过 JSON Pointer 路径更新 Surface 数据模型中的值。使�?upsert 语义�?

```typescript
interface UpdateDataModelPayload {
  surfaceId: string;
  path: string; // JSON Pointer（RFC 6901）。根路径�?"/"
  value: any; // 要写入的�?
}
```
...
| 方面 | v0.8（已废弃�?| v0.9（推荐） |
| --- | --- | --- |
| version 字段 | �?| 必须�?`"version": "v0.9"` |
| Surface 创建 | `surfaceUpdate` 时隐式创�?| 显式发�?`createSurface` |
| 组件结构 | 嵌套 `{ "Button": { props } }` | 扁平 `{ id, component: "Button", ...props }` |
| 渲染触发 | 需要额外发 `beginRendering` | `updateComponents` 收到即渲�?|
...
// 3. 触发渲染（必须显式发送，指定根组�?ID�?
{ "beginRendering": { "surfaceId": "s1", "root": "root" } }
```

> ⚠️ v0.8 �?`action.context` �?*数组**格式 `[{ key, value: { path } }]`，与 v0.9 �?*对象**格式 `{ key: { path } }` 完全不同�?
