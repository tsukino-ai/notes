...
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills-zh/use-x-chat/reference/API.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
| conversationKey | 会话唯一标识（全局唯一），用于区分不同的会�?| string | Symbol('ConversationKey') | - |
...
| parser | �?ChatMessage 转换成消费使用的 ParsedMessage，不设置时则直接消费 ChatMessage。支持将一�?ChatMessage 转换成多�?ParsedMessage | (message: ChatMessage) => BubbleMessage \| BubbleMessage[] | - | - |
...
| requestPlaceholder | 请求中的占位信息，不提供则不会展�?| ChatMessage \| (requestParams: Partial\<Input\>, info: { messages: Message[] }) => ChatMessage \|Promise\<Message\>| - | - |
...
| messages | 当前管理消息列表的内�?| MessageInfo\<ChatMessage\>[] | - | - |
...
| onRequest | 添加一�?Message，并且触发请�?| (requestParams: Partial\<Input\>, opts?: { extraInfo: AnyObject }) => void | - | - |
| setMessages | 直接修改 messages，不会触发请�?| (messages: Partial\<MessageInfo\<ChatMessage\>\>[]) => void | - | - |
| setMessage | 直接修改单条 message，不会触发请�?| (id: string \| number, info: Partial\<MessageInfo\<ChatMessage\>\>) => void | - | - |
| removeMessage | 删除单条 message，不会触发请�?| (id: string \| number) => boolean | - | - |
| queueRequest | 会将请求加入队列，等�?conversationKey 初始化完成后再发�?| (conversationKey: string \| symbol, requestParams: Partial\<Input\>, opts?: { extraInfo: AnyObject }) => void | - | - |
