...
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills-zh/use-x-chat/reference/EXAMPLES.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
const App = () => {
  ...
  const {
    ...
    defaultConversations: [{ key: 'conv-1', label: '新对�? }],
  ...
  const handleNewConversation = () => {
    ...
    addConversation({ key: newKey, label: `新对�?${conversations.length + 1}` });
...
const App = () => {
  ...
  const { messages, onRequest, setMessage, isRequesting, abort } = useXChat({
    ...
    defaultMessages: [
      ...
      {
        id: 'sys',
        message: { role: 'developer', content: '你是一个专业的前端工程师助�? },
        status: 'success',
      },
    ],
    requestFallback: (_, { error }) => ({
      content: error.name === 'AbortError' ? '已取�? : '请求失败',
      role: 'assistant',
    }),
  });
...
```tsx
// 场景：DeepSeek R1 �?reasoning_content + content 需要分开展示
