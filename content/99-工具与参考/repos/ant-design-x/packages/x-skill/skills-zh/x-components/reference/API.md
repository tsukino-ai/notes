...
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills-zh/x-components/reference/API.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
| loading | 加载状�?| boolean | - | - | 
| loadingRender | 自定义加载内容渲�?| () => React.ReactNode | - | - | 
| content | 气泡内容 | [ContentType](#contenttype) | - | - | 
| contentRender | 自定义内容渲�?| (content: ContentType, info: InfoType ) => React.ReactNode | - | - | 
| editable | 是否可编�?| boolean \| [EditableBubbleOption](#editablebubbleoption) | `false` | 2.0.0 | 
...
| streaming | 是否为流式传�?| boolean | `false` | - | 
...
`streaming` 用于通知 Bubble 当前�?`content` 是否属于流式输入的当处于流式传输模。当处于流式传输模式，无论是否启�?Bubble 输入动画，在 `streaming` 变为 `false` 之前，Bubble 不会因为把当�?`content` 全部输出完毕就触�?`onTypingComplete` 回调，只有当 `streaming` 变为 `false`，且 `content` 全部输出完毕后，Bubble 才会触发 `onTypingComplete` 回调。这样可以避免由于流式传输不稳定而导致多次触�?`onTypingComplete` 回调的问题，保证一次流式传输过程仅触发一�?`onTypingComplete`�?

在[这个例子](#bubble-demo-stream)中，你可以尝试强制关闭流式标志，同时

- 若你启用了输入动画，进行 **慢速加�?* 时，会因为流式传输的速度跟不上动画速度而导致多次触�?`onTypingComplete`�?
...
| 属�?| 说明 | 类型 | 默认�?| 版本 |
| --- | --- | --- | --- | --- |
| items | 气泡数据列表，`key`，`role` 必填。`styles`、`classNames` 会覆�?Bubble.List 对应配置。当结合X SDK [`useXChat`](/x-sdks/use-x-chat-cn) 使用时可传入`status` 帮助 Bubble 对配置进行管�?| (([BubbleProps](#bubble) & [DividerBubbleProps](#bubbledivider)) & { key: string \| number, role: string , status: MessageStatus, extraInfo?: AnyObject})[] | - | - |
...
配合 [`useXChat`](/x-sdks/use-x-chat-cn) 使用 ，`key` 可做�?`MessageId`，`extraInfo` 可作为自定义参数�?
...
**Bubble.List** �?`role` �?`items` 两个属性都可以配置气泡，其�?`role` 的配置作为默认配置使用，可缺省。`item.role` 用于指明该条数据的气泡角色，会与 `Bubble.List.role` 进行匹配。`items` 本身也可配置气泡属性，优先级高�?`role` 的配置，最终的气泡配置为：`{ ...role[item.role], ...item }`�?

注意�?**Bubble.List** 中的[语义化配置](#semantic-dom)也可以为气泡配置样式，但它的优先级最低，会被 `role` �?`items` 覆盖�?

最终配置的优先级为�?`items` > `role` > `Bubble.List.styles` = `Bubble.List.classNames`�?

特别说明，我们为 `role` 提供了四个默认字段，`ai`、`user`、`system`、`divider`。其中，`system`、`divider` 是保留字段，如果 `item.role` 赋值为它们俩之一�?*Bubble.List** 会把这条气泡数据渲染�?**Bubble.System (role = 'system')** �?**Bubble.Divider (role = 'divider')**�?

因此，若你想自定义渲染系统消息或分割线时，应该使用其他的命名�?

自定义渲染消息，可以参考[这个例子](#bubble-demo-list)�?reference 的渲染方式�?
...
| 属�?| 说明 | 类型 | 默认�?| 版本 |
| --- | --- | --- | --- | --- |
| content | 气泡内容，等�?Divider.children | [ContentType](#contenttype) | - | - |
| dividerProps | Divider 组件属�?| [Divider](https://ant.design/components/divider-cn) | - | - |
...
| components | 自定义组�?| Record<'input', ComponentType> | - | - |
| defaultValue | 输入框默认�?| string | - | - |
| disabled | 是否禁用 | boolean | false | - |
| loading | 是否加载�?| boolean | false | - |
| suffix | 后缀内容，默认展示操作按钮，当不需要默认操作按钮时，可以设�?`suffix={false}` | React.ReactNode \| false \|(oriNode: React.ReactNode,info: { components: ActionsComponents;}) => React.ReactNode \| false; | oriNode | 2.0.0 |
...
| onCancel | 点击取消按钮的回�?| () => void | - | - |
| onPaste | 粘贴回调 | React.ClipboardEventHandler<`HTMLElement`> | - | - |
| onPasteFile | 黏贴文件的回�?| (files: FileList) => void | - | - |
...
| autoSize | 自适应内容高度，可设置�?true \| false 或对象：{ minRows: 2, maxRows: 6 } | boolean \| { minRows?: number; maxRows?: number } | { maxRows: 8 } | - |
| slotConfig | 词槽配置，配置后输入框将变为词槽模式，支持结构化输入，此模式`value` �?`defaultValue` 配置将无效�?| SlotConfigType[] | - | 2.0.0 |
| skill | 技能配置，输入框将变为词槽模式，支持结构化输入，此模式`value` �?`defaultValue` 配置将无效�?| SkillType | - | 2.0.0 |
...
| 属�?| 说明 | 类型 | 默认�?| 版本 |
| --- | --- | --- | --- | --- |
| inputElement | 输入框元�?| `HTMLTextAreaElement` | - | - |
| nativeElement | 外层容器 | `HTMLDivElement` | - | - |
| focus | 获取焦点，当 `cursor = 'slot'` 时焦点会在第一个插槽类型为 `input` 的输入框内，若不存在对应�?`input` 则效果会�?`end` 一致�?| (option?: { preventScroll?: boolean, cursor?: 'start' \| 'end' \| 'all' \| 'slot' }) | - | - |
| blur | 取消焦点 | () => void | - | - |
| insert | 插入文本或者插槽，使用插槽时需确保 slotConfig 已配�?| (value: string) => void \| (slotConfig: SlotConfigType[], position: insertPosition, replaceCharacters: string, preventScroll: boolean) => void; | - | - |
| clear | 清空内容 | () => void | - | - |
...
| key | 唯一标识，type �?text 时可省略 | string | - | - |
| formatResult | 格式化最终结�?| (value: any) => string | - | 2.0.0 |
...
| 属�?              | 说明   | 类型                                  | 默认�?| 版本  |
| ------------------ | ------ | ------------------------------------- | ------ | ----- |
| props.placeholder  | 占位�?| string                                | -      | 2.0.0 |
| props.defaultValue | 默认�?| string \| number \| readonly string[] | -      | 2.0.0 |

##### select 节点属�?

| 属�?              | 说明           | 类型     | 默认�?| 版本  |
| ------------------ | -------------- | -------- | ------ | ----- |
| props.options      | 选项数组，必�?| string[] | -      | 2.0.0 |
| props.placeholder  | 占位�?        | string   | -      | 2.0.0 |
| props.defaultValue | 默认�?        | string   | -      | 2.0.0 |

##### tag 节点属�?

| 属�?       | 说明           | 类型      | 默认�?| 版本  |
| ----------- | -------------- | --------- | ------ | ----- |
| props.label | 标签内容，必�?| ReactNode | -      | 2.0.0 |
| props.value | 标签�?        | string    | -      | 2.0.0 |

##### content 节点属�?

| 属�?              | 说明   | 类型   | 默认�?| 版本  |
| ------------------ | ------ | ------ | ------ | ----- |
| props.defaultValue | 默认�?| any    | -      | 2.1.0 |
| props.placeholder  | 占位�?| string | -      | 2.1.0 |

##### custom 节点属�?

| 属�?| 说明 | 类型 | 默认�?| 版本 |
| --- | --- | --- | --- | --- |
| props.defaultValue | 默认�?| any | - | 2.0.0 |
| customRender | 自定义渲染函�?| (value: any, onChange: (value: any) => void, props: { disabled�?boolean,readOnly�? boolean},item: SlotConfigType) => React.ReactNode | - | 2.0.0 |
...
| classNames | 样式类名 | [见下](#semantic-dom) | - | - |
| closable | 是否可关�?| boolean | true | - |
| forceRender | 强制渲染，在初始化便需�?ref 内部元素时使�?| boolean | false | - |
| open | 是否展开 | boolean | - | - |
| styles | 语义化定义样�?| [见下](#semantic-dom) | - | - |
| title | 标题 | ReactNode | - | - |
...
| loading           | 加载中的开�?    | boolean                    | -      | 2.0.0 |
| defaultValue      | 默认选中状�?    | boolean                    | -      | 2.0.0 |
| value             | 开关的�?        | boolean                    | false  | 2.0.0 |
| onChange          | 变化时的回调函数 | function(checked: boolean) | -      | 2.0.0 |
| rootClassName     | 根元素样式类     | string                     | -      | 2.0.0 |

### ⚠️ 词槽模式注意事项

- **词槽模式下，`value` �?`defaultValue` 属性无�?*，请使用 `ref` 及回调事件获取输入框的值和词槽配置�?
- **词槽模式下，`onChange`/`onSubmit` 回调的第三个参数 `config`**，仅用于获取当前结构化内容�?

**示例�?*

```jsx
// �?错误用法, slotConfig �?skill 为不受控用法
...
| 属�?| 说明 | 类型 | 默认�?| 版本 |
| --- | --- | --- | --- | --- |
| items | 会话列表数据�?| `ItemType`[] | - | - |
| activeKey | 当前选中的�?| string | - | - |
| defaultActiveKey | 初始化选中的�?| string | - | - |
...
| groupable | 是否支持分组, 开启后默认�?`Conversation.group` 字段分组 | boolean \| GroupableProps | - | - |
| shortcutKeys | 快捷键操�?| { creation?: ShortcutKeys\<number\>; items?:ShortcutKeys\<'number'\> \| ShortcutKeys\<number\>[];} | - | 2.0.0 |
| creation | 新会话操作配�?| CreationProps | - | 2.0.0 |
| styles | 语义化结�?style | styles?: {creation?: React.CSSProperties;item?: React.CSSProperties;} | - | - |
| classNames | 语义化结�?className | classNames?: { creation?: string; item?:string;} | - | - |
| rootClassName | 根节点类�?| string | - | - |
...
| collapsible | 可折叠配�?| boolean \| ((group: string) => boolean) | - | - |
| defaultExpandedKeys | 默认展开或收�?| string[] | - | - |
| onExpand | 展开或收�?| (expandedKeys: string[]) => void | - | - |
| expandedKeys | 展开分组�?keys | string[] | - | - |
...
| title | 显示在提示列表顶部的标题 | React.ReactNode | - | - |
| vertical | 设置�?`true` �? 提示列表将垂直排�?| boolean | `false` | - |
| wrap | 设置�?`true` �? 提示列表将自动换�?| boolean | `false` | - |
...
| 属�?       | 说明                         | 类型            | 默认�? | 版本 |
| ----------- | ---------------------------- | --------------- | ------- | ---- |
| children    | 嵌套的子提示�?              | PromptProps[]   | -       | -    |
| description | 提示描述提供额外的信�?      | React.ReactNode | -       | -    |
| disabled    | 设置�?`true` 时禁用点击事�?| boolean         | `false` | -    |
| icon        | 提示图标显示在提示项的左�?  | React.ReactNode | -       | -    |
| key         | 唯一标识用于区分每个提示�?  | string          | -       | -    |
| label       | 提示标签显示提示的主要内�?  | React.ReactNode | -       | -    |
...
| disabled | 是否禁用 | boolean | false | - |
| maxCount | 最大上传文件数�?| number \| - | - | 2.0.0 |
| getDropContainer | 设置拖拽时，可以释放文件的区�?| () => HTMLElement | - | - |
| items | 附件列表，同 Upload `fileList` | Attachment[] | - | - |
| overflow | 文件列表超出时样�?| 'wrap' \| 'scrollX' \| 'scrollY' | - | - |
...
| imageProps | 图片属性，�?antd [Image](https://ant.design/components/image) 属�?| ImageProps | - | - |
...
更多配置请查�?[CascaderProps](https://ant.design/components/cascader-cn#api)
...
| items | 建议项列�?| SuggestionItem[] \| ((info: T) => SuggestionItem[]) | - | - |
| open | 受控打开面板 | boolean | - | - |
| rootClassName | 根元素样式类�?| string | - | - |
| onSelect | 选中建议项回�?| (value: string, selectedOptions: SuggestionItem[]) => void; | - | - |
| onOpenChange | 面板打开状态变化回�?| (open: boolean) => void | - | - |
| getPopupContainer | 菜单渲染父节点。默认渲染到 body 上，如果你遇到菜单滚动定位问题，试试修改为滚动的区域，并相对其定�?| (triggerNode: HTMLElement) => HTMLElement | () => document.body | - |
...
Suggestion 接受泛型以自定义传递给 `items` renderProps 的参数类型，当传�?`false` 时，则关闭建议面板�?

### SuggestionItem

| 属�?    | 说明           | 类型             | 默认�?| 版本 |
| -------- | -------------- | ---------------- | ------ | ---- |
| children | 子项�?        | SuggestionItem[] | -      | -    |
| extra    | 建议项额外内�?| ReactNode        | -      | -    |
| icon     | 建议项图�?    | ReactNode        | -      | -    |
| label    | 建议项显示内�?| ReactNode        | -      | -    |
| value    | 建议项�?      | string           | -      | -    |
...
| title | 状态文�?| React.ReactNode | - | - |
| icon | 状态图�?| React.ReactNode | - | - |
| loading | 加载�?| boolean \| React.ReactNode | false | - |
...
| defaultExpandedKeys | 初始化展开的节�?| string[] | - | - |
| expandedKeys | 当前展开的节�?| string[] | - | - |
...
| key | 思维节点唯一标识�?| string | - | - |
| status | 思维节点状�?| 'loading' \| 'success' \| 'error'\| 'abort' | - | - |
| title | 思维节点标题 | React.ReactNode | - | - |
| collapsible | 思维节点是否可折�?| boolean | false | - |
| blink | 闪动效果 | boolean | - | - |
...
| ----------- | ---------- | ------------------------------------------- | ------ | ---- |
| prefixCls   | 自定义前缀 | string                                      | -      | -    |
| icon        | 思维链图�?| React.ReactNode                             | -      | -    |
| title       | 思维链标�?| React.ReactNode                             | -      | -    |
| description | 思维链描�?| React.ReactNode                             | -      | -    |
| status      | 思维链状�?| 'loading' \| 'success' \| 'error'\| 'abort' | -      | -    |
...
| onClick | 组件被点击时的回调函�?| function({ item, key, keyPath, domEvent }) | - | - |
| dropdownProps | 下拉菜单的配置属�?| DropdownProps | - | - |
...
| onItemClick | 点击自定义操作按钮时的回调函�?| (info: [ItemType](#itemtype)) => void | - | - |
...
| 属�?| 说明 | 类型 | 默认�?| 版本 |
| --- | --- | --- | --- | --- |
| value | 反馈状态�?| `like` \| `dislike` \| `default` | `default` | 2.0.0 |
| onChange | 反馈状态变化回�?| (value: `like` \| `dislike` \| `default`) => void | - | 2.0.0 |

### Actions.Copy

| 属�?| 说明       | 类型            | 默认�?| 版本  |
| ---- | ---------- | --------------- | ------ | ----- |
| text | 复制的文�?| string          | ''     | 2.0.0 |
...
| 属�?  | 说明     | 类型                                     | 默认�? | 版本  |
| ------ | -------- | ---------------------------------------- | ------- | ----- |
| status | 播放状�?| 'loading'\|'error'\|'running'\|'default' | default | 2.0.0 |

### Actions.Item

| 属�?       | 说明                 | 类型                                     | 默认�? | 版本  |
| ----------- | -------------------- | ---------------------------------------- | ------- | ----- |
| status      | 状�?                | 'loading'\|'error'\|'running'\|'default' | default | 2.0.0 |
| label       | 自定义操作的显示标签 | string                                   | -       | 2.0.0 |
| defaultIcon | 默认状态图�?        | React.ReactNode                          | -       | 2.0.0 |
| runningIcon | 执行状态图�?        | React.ReactNode                          | -       | 2.0.0 |
...
| loading | 是否处于加载状�?| boolean | false | - |
| type | 文件类型 | 'file' \| 'image' \| 'audio' \| 'video' \| string | - | - |
| src | 图片或文件地址 | string | - | - |
| mask | 遮罩内容，支持函数形式获取上下文信息。对�?`type="image"`，可通过 `imageProps.preview.mask` 配置，此属性仅适用于非图像文件类型�?| React.ReactNode \| ((info: { size: string, icon: React.ReactNode, namePrefix?: string, nameSuffix?: string, name?: string, src?: string, type?: string }) => React.ReactNode) | - | - |
| icon | 自定义图�?| React.ReactNode \| PresetIcons | - | - |
| imageProps | 图片属性，�?antd [Image](https://ant.design/components/image-cn#api) 属�?| ImageProps | - | - |
| videoProps | 视频属性配�?| Partial<React.JSX.IntrinsicElements['video']> | - | - |
| audioProps | 音频属性配�?| Partial<React.JSX.IntrinsicElements['audio']> | - | - |
| spinProps | 加载中属�?| [SpinProps](https://ant.design/components/spin-cn#api) & { showText?: boolean; icon?: React.ReactNode } | - | - |
...
| size      | 卡片大小     | 'small' \| 'default'                          | 'default' | -    |
| removable | 是否可删�?  | boolean \| ((item: FileCardProps) => boolean) | false     | -    |
...
| popoverOverlayWidth | 弹出层宽�?| number \| string | 300 | - |
...
| highlightProps | 代码高亮配置，透传�?react-syntax-highlighter | [`SyntaxHighlighterProps`](https://github.com/react-syntax-highlighter/react-syntax-highlighter?tab=readme-ov-file#props) | - |
| prismLightMode | 是否使用 Prism 轻量模式，根�?`lang` 自动按需加载语言支持以减少打包体�?| `boolean` | `true` |
...
| actions | 操作栏配�?| `{ enableZoom?: boolean; enableDownload?: boolean; enableCopy?: boolean; customActions?: ItemType[] }` | `{ enableZoom: true, enableDownload: true, enableCopy: true }` |
| onRenderTypeChange | 渲染类型切换回调 | `(value: 'image' \| 'code') => void` | - |
| prefixCls | 样式前缀 | `string` | - |
| style | 自定义样�?| `React.CSSProperties` | - |
...
| 属�?| 说明 | 类型 | 默认�?| 版本 |
| --- | --- | --- | --- | --- |
| permission | 表明当前用户是否授予当前来源（origin）显�?web 通知的权限�?| NotificationPermission | - | - |
...
�?Windows 系统上不同版本系统的设置路径会有不同，可大概参考路径：“开始”菜�?> “设置�? “系统�?> 然后在左侧选择 “通知和操作”，之后可以对全局通知以及应用通知等进行操作�?

### �?Mac 上更�?`通知` 设置

�?Mac 上，使用 ”通知�?设置来指定不想被通知打扰的时段，并控制通知�?”通知中心�?中的显示方式。若要更改这些设置，请选取 ”苹果“菜�? ”系统设置“，然后点按边栏中的 ”通知”（你可能需要向下滚动）�?

## FAQ

### 已经获取了当前来�?`origin` 显示系统通知的权限，`onShow` 回调也触发了，为何还是无法展示推送的通知�?
...
| 属�?| 说明 | 类型 | 默认�?| 版本 |
| --- | --- | --- | --- | --- |
| treeData | 文件树数�?| [FolderTreeData](#foldertreenode)[] | `[]` | - |
| selectable | 是否开启选择功能 | boolean | `true` | - |
| selectedFile | 选中的文件路径（受控�?| string[] | - | - |
| defaultSelectedFile | 默认选中的文件路�?| string[] | `[]` | - |
| onSelectedFileChange | 文件选择变化时的回调 | (file: { path: string[]; name?: string; content?: string }) => void | - | - |
| directoryTreeWith | 目录树宽�?| number \| string | `278` | - |
| emptyRender | 空状态时的展示内容，设为 `false` 时不展示 | false \| React.ReactNode \| (() => React.ReactNode) | - | - |
| previewRender | 自定义文件预览内�?| React.ReactNode \| ((file: { content?: string; path: string[]; title?: React.ReactNode; language: string }, info: { originNode: React.ReactNode }) => React.ReactNode) | - | - |
| expandedPaths | 展开的节点路径数组（受控�?| string[] | - | - |
| defaultExpandedPaths | 默认展开的节点路径数�?| string[] | - | - |
| defaultExpandAll | 是否默认展开所有节�?| boolean | `true` | - |
...
| onFolderClick | 文件夹点击事�?| (folderPath: string) => void | - | - |
