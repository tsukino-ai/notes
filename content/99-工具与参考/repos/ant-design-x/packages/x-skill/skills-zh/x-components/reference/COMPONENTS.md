...
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills-zh/x-components/reference/COMPONENTS.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
**Purpose**: Display a single chat message â€?AI reply or user message.
...
// Streaming mode â€?set streaming=false on final chunk
<Bubble
...
| `typing` | Typing animation â€?`false` to disable, `{ effect: 'typing' \| 'fade-in', step, interval }` to configure |
...
| `contentRender` | Custom renderer for `content` â€?use for Markdown or chart components |
| `editable` | Enable inline content editing |

**`Bubble.List` â€?the preferred multi-message component**:
...
| `role`       | Map of role name â†?default props for that role                             |
...
**Purpose**: Chat input box â€?text entry, file attachment, voice input, submit.
...
| `submitType`            | `'enter'` (default) \| `'shiftEnter'` â€?what triggers submit |
...
| Prop | Purpose |
| --- | --- |
| `items` | Array of `ConversationItemType` â€?`key`, `label`, `icon`, `group`, `disabled` |
...
| Prop | Purpose |
| --- | --- |
| `items` | Array of `PromptProps` â€?`key`, `label`, `description`, `icon`, `disabled`, `children` |
...
**Purpose**: Visualize a multi-step agent reasoning chain â€?tool calls, searches, sub-tasks.
...
**Purpose**: Collapsible "deep thinking" block â€?for models that expose a reasoning step.
...
**Purpose**: Feedback/control buttons below an AI message â€?copy, thumbs up/down, retry, audio.
...
| Component          | Purpose                                         |
| ------------------ | ----------------------------------------------- |
| `Actions.Copy`     | Copy-to-clipboard button â€?prop `text`          |
| `Actions.Feedback` | Like/dislike toggle â€?props `value`, `onChange` |
| `Actions.Audio`    | Play/loading/error audio button â€?prop `status` |
...
**Purpose**: File attachment list with drag-drop, paste, and overflow support â€?used in `Sender.Header`.
...
| Prop | Purpose |
| --- | --- |
| `items` | Array of `SourcesItem` â€?`key`, `title`, `url`, `icon`, `description` |
| `title` | Section heading |
...
| `children` | Render prop â€?`({ onTrigger, onKeyDown }) => ReactElement` |
...
**Purpose**: Display a hierarchical file/folder tree with optional file preview â€?useful in AI coding assistant scenarios.
...
| Prop | Purpose |
| --- | --- |
| `treeData` | Tree structure â€?`FolderTreeData[]` with `title`, `path`, `content`, `children` |
...
**Purpose**: Global configuration wrapper â€?replaces `antd`'s `ConfigProvider`.
...
- Fully extends `antd`'s `ConfigProvider` â€?all antd config props are valid.
- Adds x-specific locale strings via `@ant-design/x/locale/zh_CN` (or `en_US`).
- Place **once at the app root** â€?do not nest multiple `XProvider` instances unnecessarily.
