---
title: x-chat-provider 实战示例
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills-zh/x-chat-provider/reference/EXAMPLES.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
// DeepSeek 格式（注�?Output 类型�?Partial<Record<SSEFields, XModelResponse>>�?
...
适用于不需要格式转换、在 `Bubble.List` �?`contentRender` 中自行渲染的场景�?
...
// ChatMessage �?ChatOutput | ChatInput 的联合类�?
...
const App = () => {
  const { messages, onRequest, isRequesting, abort } = useXChat({
    provider,
    requestPlaceholder: {
      choices: [{ message: { content: '等待�?..', role: 'assistant' } }],
    },
    requestFallback: {
...
const provider = new OpenAIChatProvider({
  request: XRequest<XModelParams, XModelResponse, XModelMessage>(BASE_URL, {
    manual: true,
    callbacks: {
      // 流式更新时触发，message 是当�?MessageInfo（含 transformMessage 结果�?
...
const provider = new OpenAIChatProvider({
  request: XRequest(BASE_URL, {
    manual: true,
    retryInterval: 3000, // 失败�?3 秒重�?
    retryTimes: 3, // 最多重�?3 �?
    callbacks: {
      onError: (error) => {
        // 主动 abort 不重试；返回数字可覆�?retryInterval
        if (error.name === 'AbortError') return;
        return 5000; // 动态设置重试间隔为 5 �?
      },
    },
    params: { model: 'gpt-4o', stream: true },
  }),
...
## 场景7：多会话 Provider 工厂（配�?useXConversations�?

```ts
import { OpenAIChatProvider, XRequest } from '@ant-design/x-sdk';
import type { XModelParams, XModelResponse } from '@ant-design/x-sdk';

// 每个会话独立一�?Provider 实例，避免状态混�?
