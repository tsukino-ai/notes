---
title: Data Binding Reference
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills/x-card/reference/DATA_BINDING.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
**Absolute paths** start with `/` ‚Ä?always resolve from model root. **Relative paths** (no leading `/`) ‚Ä?resolve relative to collection scope inside a template.
...
```typescript
// Literal ‚Ä?static, does not react to data changes
{ id: 'title', component: 'Text', text: 'Hello World' }

// Data-bound ‚Ä?reads from /user/name, re-renders on change
...
> ‚ö†Ô∏è Server sync happens only on explicit action (button click), not on every keystroke.
...
Send targeted `updateDataModel` commands to update only what changed ‚Ä?no need to resend component structure:
...
- Send granular updates targeting only changed paths ‚Ä?not full model replacements on each change
