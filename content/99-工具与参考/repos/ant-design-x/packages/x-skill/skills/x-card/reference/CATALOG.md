...
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills/x-card/reference/CATALOG.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
> ‚ö†Ô∏è `catalogId` URIs are identifiers, not runtime-fetched resources in production. For custom catalogs, always use `registerCatalog()` to avoid network dependency.
...
| ------------------------------------------------ | --------------------------------- |
| Add/remove container component (Grid, Accordion) | **Breaking** ‚Ä?major version bump |
...
```
local://coffee_booking_catalog.json   ‚Ü?local registry
https://company.com/catalogs/v2/catalog.json   ‚Ü?remote, versioned URI
```
...
- Unknown component type ‚Ü?render a placeholder text, not a crash
- Unknown prop on known component ‚Ü?silently ignored, component renders normally
- Removed component ‚Ü?no longer sent, client unaffected
