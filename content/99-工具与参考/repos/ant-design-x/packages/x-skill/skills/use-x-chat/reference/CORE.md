...
author: ant-design
date: 2026-05-28
source: https://github.com/ant-design/x/blob/main/packages/x-skill/skills/use-x-chat/reference/CORE.md
tags:
  - react
  - ai-ui
  - ant-design-x
...
> ‚ö†Ô∏è `onReload`'s second parameter is `requestParams`; usually pass `{}` (original context will be reused)
...
> ‚ö†Ô∏è **Common mistake**: `Bubble.List` uses the `role` prop, not `roles`
...
// ‚ù?Wrong (roles is not a valid prop)
...
items={messages.map(({ id, message, status }) => ({
  key: id,
  role: message.role,    // 'user' | 'assistant' | 'system' ‚Ä?must match role config keys
  content: message.content,
  loading: status === 'loading',
