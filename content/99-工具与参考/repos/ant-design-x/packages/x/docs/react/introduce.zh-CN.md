...
**`@ant-design/x-card` - 动态卡片渲染器**

基于 A2UI 协议的动态卡片渲染组件，让 AI Agent 通过结构化 JSON 消息流动态构建交互式界面。支持流式渲染、数据绑定和响应式更新，适用于 AI 助手界面、智能表单、数据可视化等场景。查看详情请点击[这里](/x-cards/introduce-cn)。
