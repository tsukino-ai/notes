---
title: @mastra/daytona
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/workspaces/daytona/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
For one-shot tasks â€?sandbox is deleted immediately on stop:
...
// Later â€?reconnects to the same sandbox
...
The simplest way â€?filesystems are mounted automatically when the sandbox starts:
...
//
// 1. **Hello, world!** â€?Successfully printed the message.
// 2. **Current Working Directory** â€?`/home/daytona`
//
...
| `snapshot`            | `string`  | â€?                    | Pre-built snapshot ID. Takes precedence over `image`.                                                                                        |
| `image`               | `string`  | â€?                    | Docker image for sandbox creation. Triggers image-based creation when set. Can be combined with `resources`. Ignored when `snapshot` is set. |
...
| `volumes`             | `array`   | â€?                    | `[{ volumeId, mountPath }]`                                                                                                                  |
| `networkBlockAll`     | `boolean` | `false`               | Block all network access                                                                                                                     |
| `networkAllowList`    | `string`  | â€?                    | Comma-separated allowed CIDR addresses                                                                                                       |
