---
title: @mastra/google-drive
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/workspaces/google-drive/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- **`accessToken`** â€?A pre-obtained OAuth access token (use the `https://www.googleapis.com/auth/drive` scope).
- **`getAccessToken`** â€?A callback that returns a token; useful when tokens are refreshed externally.
- **`serviceAccount`** â€?A Google service account. Share the target folder with the service account email.
