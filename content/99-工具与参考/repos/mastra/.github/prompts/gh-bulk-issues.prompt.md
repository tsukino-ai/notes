---
title: Bulk Issue Solver
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.github/prompts/gh-bulk-issues.prompt.md
tags:
  - agent-framework
  - typescript
  - mastra
...
Orchestrate parallel `mc` (mastracode) headless instances to debug and fix multiple GitHub issues simultaneously. You act as the supervisor â€?spawning workers, monitoring progress, reviewing output, and creating PRs.
...
4. If the instance **completed its work** (code + tests + changeset):
   - Review the diff â€?does the fix make sense?
...
- **All `mc` instances can run in parallel** â€?they're IO-bound, not CPU-bound
- **One worktree per issue, always** â€?every task related to an issue (debugging, PR comments, CI fixes) must run in that issue's worktree. Never create a second worktree for the same issue. The worktree accumulates context (commits, diffs, build artifacts) that each `mc` instance benefits from.
- **Always restart timed-out processes** with a continuation prompt that includes context from the diff and last output
- **Never leave a process unmonitored** â€?check every 3 minutes
- **Update report files continuously** so the user always has a written record
- **Review all `mc` output before creating PRs** â€?subagent work is untrusted
