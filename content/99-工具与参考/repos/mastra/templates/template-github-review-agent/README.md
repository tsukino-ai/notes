...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/templates/template-github-review-agent/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- `SMALL_PR_MAX` / `MEDIUM_PR_MAX` â€?PR size breakpoints for adaptive review depth.
- `SKIP_PATTERNS` â€?Regex patterns for files to skip during review.
- `MIN_DELETION_ONLY_LINES` â€?Threshold for skipping deletion-only files.

Batch sizes for the workflow are configured at the top of `src/mastra/workflows/pr-review-workflow.ts`:

- `BATCH_CHAR_BUDGET` â€?Max total characters per agent call (default: 400k).
- `BATCH_FILE_LIMIT` â€?Max files per agent call (default: 40).
