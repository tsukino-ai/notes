---
title: Evals With Memory
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/examples/evals-with-memory/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
has **memory** turned on â€?including observational-memory in `thread` scope
...
resolution â€?only `args.memory.thread` does (`resolveThreadIdFromArgs`
...
**not** pass any `memory` option to `agent.generate()` â€?only
...
- Pre-setting `RequestContext.MastraMemory` (the trick used inside
  ...
  the agent a thread â€?it's an internal contract populated by
