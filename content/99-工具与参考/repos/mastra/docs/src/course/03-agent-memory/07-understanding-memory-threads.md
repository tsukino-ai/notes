...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/docs/src/course/03-agent-memory/07-understanding-memory-threads.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- **One thread â†?One owner** (each thread has exactly one `resourceId` that identifies its owner)
- **One resource â†?Many threads** (a user can have multiple separate conversations)
- **One thread â†?Messages with different resourceIds** (messages can have varying `resourceId` values for attribution and filtering purposes)
