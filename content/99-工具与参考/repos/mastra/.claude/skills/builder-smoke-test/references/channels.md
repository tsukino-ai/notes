...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/builder-smoke-test/references/channels.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- [ ] `providers` is `[]` (empty array) â€?`slack` is filtered out by `isConfigured`
- [ ] Section status: âœ?(negative path verified). Steps 2â€? (positive Slack flow) defer to Run 2 only when `SLACK_*` env vars are set.
