---
title: Stored Agent CRUD & Skill Attachment
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/builder-smoke-test/references/agents.md
tags:
  - agent-framework
  - typescript
  - mastra
...
The model values used below (`openai/gpt-4o-mini`, `openai/gpt-4o`) are valid under the scaffolded project's admin policy, which allows any `openai` model via wildcard plus exactly `anthropic/claude-opus-4-7` (see the scaffolded project's `src/mastra/index.ts` â†?`models.allowed`). If you've changed the policy, swap these for something allowed.

> **Server schema reminder:** `model` is `{ provider, name }` â€?**not** `{ provider, modelId }`. The schema lives in `packages/server/src/server/schemas/stored-agents.ts`. Posting `modelId` returns `400 model: Invalid input`.
...
> **Capability gate.** Create / PATCH / DELETE steps require `stored-agents:write`. The scaffold grants this to owner, admin, and member; viewer does not have it. Under `--role viewer`, mark write steps `n/a â€?role lacks stored-agents:write` and run only the read-side steps (GET list, GET by id).
...
```bash
curl -s -o /dev/null -w "%{http_code}\n" -X DELETE $BASE/stored/agents/$AGENT_ID  # â†?200
curl -s -o /dev/null -w "%{http_code}\n" -X DELETE $BASE/stored/skills/$SKILL_ID  # â†?200
curl -s -o /dev/null -w "%{http_code}\n" $BASE/stored/agents/$AGENT_ID            # â†?404
...
- [ ] Auth off: behaves as owner (no caller â†?bypass)
