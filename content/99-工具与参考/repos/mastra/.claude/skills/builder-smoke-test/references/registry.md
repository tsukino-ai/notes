---
title: Skill Registry (skills.sh + Library Copy)
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/builder-smoke-test/references/registry.md
tags:
  - agent-framework
  - typescript
  - mastra
...
1. **External registry** ‚Ä?currently only `skills.sh`, opt-in via `builder.registries.skillsSh.enabled`. Browse + install proxies to skills.sh and persists a stored skill with `metadata.origin = { type: 'skills-sh', ... }`.
2. **Library Copy** ‚Ä?any authenticated user can copy a public stored skill they don't own. The copy is a fresh private stored skill with `metadata.origin = { type: 'library-copy', sourceSkillId, sourceAuthorId, copiedAt }`.
...
Pick a skill from search/popular. Preview takes the GitHub coordinates as query params (`owner`, `repo`, `path` ‚Ä?where `path` is the skill name within the repo). Source: `packages/server/src/server/schemas/builder-registry.ts` `builderRegistryPreviewQuerySchema`.
...
- [ ] On collision, toast offers "Open existing" ‚Ü?navigates to the stored skill
...
> 3. **Skip:** under `--auth off`, every API-created skill resolves to the same `null` author, so the Copy affordance won't appear naturally. If you didn't seed and don't have a second account, mark these steps `n/a ‚Ä?multi-user data not available` and move on; do not flag them as failures.

### 9. Library page lists public skills

Navigate to `/agent-builder/library`.

- [ ] Shows at least one public skill authored by someone other than current user (skip with a note if none exist ‚Ä?see setup note above)
...
- [ ] Click "Copy" ‚Ü?name prompt dialog appears
- [ ] Default name is `<source-name>-copy`
- [ ] Submit ‚Ü?new private stored skill created
