...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/builder-smoke-test/references/model-policy.md
tags:
  - agent-framework
  - typescript
  - mastra
...
> **Non-admin runs.** The scaffold grants `member` write on stored agents, so the create-time policy checks (steps 2â€?) are reachable from `--role member`. `viewer` gets 403 before policy validation runs â€?for viewer, only step 1 (read-side) is meaningful; the UI dropdown gating in `references/ui.md` is the alternative path for viewer model-policy verification.
...
> **Capability gate:** Steps 2â€? require `stored-agents:write`. For `--role viewer`, skip them and rely on the UI dropdown check in `references/ui.md`. For `--role admin` and `--role member`, run them all (the scaffold grants member that perm).
