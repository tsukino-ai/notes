...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/builder-smoke-test/references/ui.md
tags:
  - agent-framework
  - typescript
  - mastra
...
Browser-based verification of the Agent Builder UI. Use whichever browser tool the harness has wired up (Stagehand, Chrome MCP, etc.). If none is available, skip this section with `--skip-browser` and report â­ï¸.
...
- **Core** (steps 1â€?): shell loads, skills list, skills create starter,
  skill edit page, agent list, agent view page, star toggle, role
  impersonation menu (admin/owner only).
- **Extended** (steps 9â€?5): model dropdown, workspace dropdown, Library
  - Copy flow, registry button gating, origin badges, mobile bottom-bar
    parity, scrollable list layout.

If you skip a step, mark it â­ï¸ in the result table with a one-line reason
...
- [ ] Canonical detail routes are `/agent-builder/skills/<id>/edit` (owner) and `/agent-builder/skills/<id>/view` (non-owner). The bare `/agent-builder/skills/<id>` path redirects to `/edit` â€?prefer navigating via the list, since the redirect target doesn't depend on ownership.
...
- [ ] Four example prompt cards are visible (e.g. Code reviewer, Doc summarizer, Onboarding tutor, Research notes â€?log any drift in labels)
...
- [ ] No manual create dialog / no Name+Description form here â€?the flow is AI-first
...
- [ ] Right panel: skill details form â€?Name, Description, Instructions
- [ ] No explicit Save button visible â€?saving is autosaved (look for a saving/saved indicator near the form)
...
- [ ] Visibility selector: not visible under `--auth off` (server forces public). Under `--auth on`, the selector renders in the page-header action group on `lg` viewports and above (â‰?024px) via `VisibilitySelectConnected`. Below `lg` it moves into the mobile menu (`SkillBuilderMobileMenu` with `showSetVisibility`). Resize the viewport or open the mobile menu to confirm if the desktop slot looks empty.
...
- [ ] Clicking a row navigates to `/agent-builder/agents/<id>/view` (NOT `/edit`). Skills list rows go to `/edit`; agents list rows go to `/view`. This asymmetry is intentional â€?log it if it's reversed.
...
- [ ] Bottom: `Message your agent...` chat input â€?agents are runnable from view
...
Open the user menu â†?the picker label is `PREVIEW AS ROLE` (a section header in the user menu). Each role is its own menu item directly under that header.

- [ ] Picker offers only roles **different from the current role**. Logged in as admin, you'll see `Member` and `Viewer` (no `Admin` item â€?admin is the implicit baseline). This is intentional.
- [ ] Selecting `Viewer`:
  ...
  - [ ] `Infrastructure` sidebar entry remains visible â€?viewer permissions include `*:read`, which matches `infrastructure:read`. The infra page itself is read-only for viewers. Not a regression.
...
> **Important:** impersonation is UI-only. The API still answers per the _real_ logged-in role. If you `curl` the same endpoint while impersonating viewer, you'll still get the admin's response. That's expected â€?record it that way in the report, don't file it as a bug.

### 8b. Run 3 / non-admin UI parity _(Core, non-admin runs only)_

Run this **instead of** step 8 under `--auth on --role member` or `--role viewer`. Step 8 (role impersonation) is admin-only â€?when the live user is a non-admin, the picker is hidden and there's nothing to impersonate.
...
- [ ] `Infrastructure` page renders for both member and viewer (read-only surface â€?deployment-shape data, no secrets).
...
- [ ] Under `--auth on`, clicking a row you don't own should navigate to `/agent-builder/skills/<id>/view` (read-only). Under `--auth off` everyone is treated as owner so the navigation lands on `/edit` instead â€?log which path you observed.
...
- [ ] Bottom-bar shows the same primary entries as the desktop sidebar (Agents, Skills, Favorites, Library, Infrastructure â€?Infrastructure is read-only for all default roles since the payload is deployment-shape with no secrets)
