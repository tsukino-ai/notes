...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/builder-smoke-test/references/permissions.md
tags:
  - agent-framework
  - typescript
  - mastra
...
**There is no server-side "preview as role" header in this build.** The "View as role" feature in the UI is purely frontend state (see `references/ui.md` â€?Impersonation UI). It does not change what the API returns. To exercise role gating at the API layer, the logged-in user must actually have that role.
...
If mismatch, halt â€?see `references/auth.md` step 1b.
...
- [ ] `POST /stored/agents` â†?200 (has `stored-agents:write`)
- [ ] `POST /agents/:id/chat` against an existing public agent â†?200 (has `:execute`)
- [ ] `POST /stored/skills/:id/publish` against own draft â†?403 (no `:publish`)
- [ ] `PATCH /stored/skills/:id` on another author's row â†?403 (ownership check, not perm check)

If `--role viewer`:

- [ ] `POST /agents/:id/chat` â†?403 (no `:execute`)
- [ ] `POST /stored/agents` â†?403 (no `:write`)

### 5. Delete is owner-only

The scaffold maps `admin` â†?`['*']`, so an admin in this project passes DELETE checks (owner-equivalent). Only `viewer` and `member` see 403 on DELETE.

- [ ] `--role owner` or `--role admin`: `DELETE /stored/agents/:id` on own â†?200
- [ ] `--role member`: same DELETE â†?403
- [ ] `--role viewer`: same DELETE â†?403

If the matrix and the live response disagree, that is the finding â€?file it, don't "correct" the matrix without checking the scaffold's `roleMapping` in `auth.ts` first.
...
If `--role admin` (or `owner`), also run the **UI Impersonation** subset in `references/ui.md` â€?that's the only honest way to exercise viewer/member UI gating without re-authenticating.
...
- [ ] Execute pass/fail per the matrix (member â‰?viewer)
