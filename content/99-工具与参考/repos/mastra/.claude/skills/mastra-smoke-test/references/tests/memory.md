---
title: Memory Testing (`--test memory`)
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/mastra-smoke-test/references/tests/memory.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- [ ] Navigate back to `/agents` â†?same agent
...
deprecated `/generate-legacy` route â€?sending them to `/generate` silently
...
`{ threads: [...], total, page, perPage, hasMore }` â€?**not** a bare array.
...
`resourceid` instead of `resourceId` â€?the unknown param is dropped and no
filter is applied.
