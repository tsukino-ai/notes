---
title: MCP Servers Testing (`--test mcp`)
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/mastra-smoke-test/references/tests/mcp.md
tags:
  - agent-framework
  - typescript
  - mastra
...
â€?that's the Studio route). The `MCP` here refers to Mastra hosting MCP
...
- Hitting `/api/mcps` or `/api/mcp/servers` â€?neither exists server-side
- Treating an empty `servers` list as failure on the default template â€?
  it's expected
