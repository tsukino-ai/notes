---
title: Tools Testing (`--test tools`)
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/mastra-smoke-test/references/tests/tools.md
tags:
  - agent-framework
  - typescript
  - mastra
...
Note the `data` wrapper â€?the tool's input schema fields go **inside** `data`,
...
- Using the export name (e.g. `weatherTool`) instead of the tool's `id`
  (e.g. `get-weather`) â†?404 "Tool not found"
- Sending input fields at the top level instead of under `data` â†?validation
