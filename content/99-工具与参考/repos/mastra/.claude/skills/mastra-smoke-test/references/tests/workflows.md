---
title: Workflows Testing (`--test workflows`)
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/mastra-smoke-test/references/tests/workflows.md
tags:
  - agent-framework
  - typescript
  - mastra
...
Note the `inputData` wrapper â€?the workflow's input schema fields go
...
- Response typically includes a `traceId` â€?capture it to cross-reference in
  `traces.md` verification

**Common mistakes:**

- Sending input fields at the top level instead of under `inputData` â†?
  HTTP 500 "Invalid input data: `<field>` expected `<type>`, received
  undefined"
- Using the workflow's display name instead of its registered id â†?404
  "Workflow not found"
