...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/.claude/skills/mastra-smoke-test/references/cloud-deploy.md
tags:
  - agent-framework
  - typescript
  - mastra
...
**‚ö†Ô∏è Always warn the user before running this** ‚Ä?it opens a browser:
...
**Critical step** ‚Ä?verifies the full trace pipeline works:

1. Make a Server API call (using script or curl)
2. Return to Studio UI ‚Ü?Observability ‚Ü?Traces
3. Refresh the page
...
WorkOS tokens expire after 5 minutes. Check token validity before re-logging in ‚Ä?use the refresh token first. See the `get_valid_token` helper in `references/tests/traces.md`.
...
```typescript
server: {
  apiRoutes: [helloRoute],  // ‚ú?Correct
  // routes: [helloRoute],  // ‚ù?Wrong - silently fails
}
```
