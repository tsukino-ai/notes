---
title: @mastra/otel-exporter
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/observability/otel-exporter/CHANGELOG.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- Added log export to `@mastra/otel-exporter`. Logs emitted on the Mastra observability bus are now forwarded to the configured OTLP endpoint alongside traces, using the same provider configuration. ([#13529](https://github.com/mastra-ai/mastra/pull/13529))
  ...
  **Other improvements:**
  ...
  - Calling `flush()` or `shutdown()` immediately after init no longer drops telemetry â€?teardown waits for setup to finish before draining providers.
...
- Renamed emitted OTel GenAI cache usage attributes to match the OpenTelemetry semantic conventions: ([#15966](https://github.com/mastra-ai/mastra/pull/15966))
  - `gen_ai.usage.cached_input_tokens` â†?`gen_ai.usage.cache_read.input_tokens`
  - `gen_ai.usage.cache_write_tokens` â†?`gen_ai.usage.cache_creation.input_tokens`
...
- dependencies updates: ([#15526](https://github.com/mastra-ai/mastra/pull/15526))
  - Updated dependency [`@opentelemetry/api@^1.9.1` â†—ï¸Ž](https://www.npmjs.com/package/@opentelemetry/api/v/1.9.1) (from `^1.9.0`, in `dependencies`)
  - Updated dependency [`@opentelemetry/core@^2.7.0` â†—ï¸Ž](https://www.npmjs.com/package/@opentelemetry/core/v/2.7.0) (from `^2.6.0`, in `dependencies`)
  - Updated dependency [`@opentelemetry/resources@^2.7.0` â†—ï¸Ž](https://www.npmjs.com/package/@opentelemetry/resources/v/2.7.0) (from `^2.6.0`, in `dependencies`)
  - Updated dependency [`@opentelemetry/sdk-trace-base@^2.7.0` â†—ï¸Ž](https://www.npmjs.com/package/@opentelemetry/sdk-trace-base/v/2.7.0) (from `^2.6.0`, in `dependencies`)
  - Updated dependency [`@opentelemetry/sdk-trace-node@^2.7.0` â†—ï¸Ž](https://www.npmjs.com/package/@opentelemetry/sdk-trace-node/v/2.7.0) (from `^2.6.0`, in `dependencies`)
...
- Upgraded Langfuse integration to the official v5 SDK, replacing the deprecated v3 package. ([#14985](https://github.com/mastra-ai/mastra/pull/14985))
  ...
  **New features:**
  - Access Langfuse's full platform via `exporter.client` â€?prompt management, datasets, evaluations, and scoring
  - New `environment` and `release` config options for filtering traces in the Langfuse dashboard

  **No breaking changes to your existing code** â€?`LangfuseExporter`, `LangfuseExporterConfig`, and `withLangfusePrompt()` work the same way. Just upgrade the package and your traces will use the latest Langfuse format.

  **Note:** `withLangfusePrompt({ id })` is deprecated â€?Langfuse v5 requires `name` + `version` for prompt linking.
...
- Rename LLM span types and attributes to use Model prefix ([#9105](https://github.com/mastra-ai/mastra/pull/9105))
  ...
  - `AISpanType.LLM_GENERATION` â†?`AISpanType.MODEL_GENERATION`
  - `AISpanType.LLM_STEP` â†?`AISpanType.MODEL_STEP`
  - `AISpanType.LLM_CHUNK` â†?`AISpanType.MODEL_CHUNK`
  - `LLMGenerationAttributes` â†?`ModelGenerationAttributes`
  - `LLMStepAttributes` â†?`ModelStepAttributes`
  - `LLMChunkAttributes` â†?`ModelChunkAttributes`
  - `InternalSpans.LLM` â†?`InternalSpans.MODEL`
