---
title: OtelExporter - OpenTelemetry Exporter
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/observability/otel-exporter/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
> **‚ö†Ô∏è Important:** This package requires you to install an additional exporter package based on your provider. Each provider section below includes the specific installation command.

## Signals

The exporter forwards two OpenTelemetry signals:

- **Traces** ‚Ä?Mastra spans, exported via `BatchSpanProcessor` over an OTLP `SpanExporter`.
- **Logs** ‚Ä?Mastra log events, exported via `BatchLogRecordProcessor` over an OTLP `LogRecordExporter`. When a log carries `traceId`/`spanId`, both the OTEL log record's native trace context and `mastra.traceId`/`mastra.spanId` attributes are populated so backends like Grafana, Datadog, and Honeycomb can correlate logs to traces automatically.
...
Log export requires the matching OTLP log exporter package for your protocol ‚Ä?install one of:
