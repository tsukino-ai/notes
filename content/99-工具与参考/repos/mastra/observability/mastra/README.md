...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/observability/mastra/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
Central event router that dispatches tracing, metric, and log events to registered exporters. All handler promises are tracked for reliable flush and shutdown â€?no events are silently dropped.
...
- **Which agent is expensive?** â†?group by `entity_name` where `entity_type=agent`
- **Why is this tool slow only sometimes?** â†?group by `parent_name`
- **What's the total cost of this user-facing flow?** â†?group by `root_name`
- **Which model is cheapest for this agent?** â†?group by `model` where `entity_name=X`
