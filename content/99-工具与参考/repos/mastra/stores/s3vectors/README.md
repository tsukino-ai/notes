---
title: @mastra/s3vectors
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/stores/s3vectors/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
> âš ï¸ Amazon S3 Vectors is a Preview service.
...
- Rich **metadata filtering** with JSON operators (`$and`, `$or`, `$in`, â€?
- IAM/SCP-based access control in the `s3vectors` namespace
- Automatic optimization for writes/updates/deletes as data scales

## Methods

- `connect(): Promise<void>` / `disconnect(): Promise<void>` â€?No-ops for interface parity (disconnect closes the underlying HTTP handler)
- `createIndex({ indexName, dimension, metric? })` â†?`Promise<void>`
  _After creation, index name/dimension/metric/non-filterable keys cannot be changed._
- `upsert({ indexName, vectors, metadata?, ids? })` â†?`Promise<string[]>`
  _Dimensions are validated against the index; Dates in metadata are serialized to epoch millis._
- `query({ indexName, queryVector, topK?, filter?, includeVector? })` â†?`Promise<QueryResult[]>`
  _`score` is derived from distance so that â€œhigher is betterâ€?_
- `updateVector({ indexName, id, update: { vector?, metadata? } })` â†?`Promise<void>`
  _Performs Getâ†’mergeâ†’Put (Put is full replace)._
- `deleteVector({ indexName, id })` â†?`Promise<void>`
- `listIndexes()` â†?`Promise<string[]>`
- `describeIndex({ indexName })` â†?`Promise<IndexStats>`
  _Returns `{ dimension, metric, count }`._
- `deleteIndex({ indexName })` â†?`Promise<void>`

## Related Links

- Amazon S3 Vectors â€?Working with vector buckets & indexes:  
