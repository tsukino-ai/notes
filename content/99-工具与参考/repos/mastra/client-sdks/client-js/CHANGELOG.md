---
title: @mastra/client-js
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/client-sdks/client-js/CHANGELOG.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- Fixed `clientTools` being silently dropped â€?and never executed â€?on thread-backed chats. When a chat had a `threadId`, the React `useChat` hook routed messages through the new agent signals path but did not pass the `clientTools` map into the signal startup flow, so client-side tools were unavailable when the model requested them. ([#16540](https://github.com/mastra-ai/mastra/pull/16540))
...
- Fix client-js bugs surfaced by the SDK â†?server contract audit. ([#16439](https://github.com/mastra-ai/mastra/pull/16439))
...
- Add clone agent feature. Code-defined or stored agents can be cloned into new stored agents via the client SDK: ([#12861](https://github.com/mastra-ai/mastra/pull/12861))
  ...
  The clone serializes the agent's full resolved configuration â€?model, instructions, tools, workflows, sub-agents, memory, input/output processors, and scorers â€?using the caller's `requestContext` and persists it as a new stored agent.
...
- Fixed multiple issues with stored agents: ([#12861](https://github.com/mastra-ai/mastra/pull/12861))
  ...
  **Before:**
  ...
  await client.updateStoredAgent(agentId, {
    memory: null, // â?Would throw validation error
  });
  ...
  **After:**
  ...
  // Can disable memory
  await client.updateStoredAgent(agentId, {
    memory: null, // âœ?Works, disables memory
  });
...
- 6690a16: dependencies updates:
  - Updated dependency [`@ag-ui/client@^0.0.35` â†—ï¸Ž](https://www.npmjs.com/package/@ag-ui/client/v/0.0.35) (from `^0.0.27`, in `dependencies`)
...
- 2cf3b8f: dependencies updates:
  ...
  - Updated dependency [`zod-to-json-schema@^3.24.5` â†—ï¸Ž](https://www.npmjs.com/package/zod-to-json-schema/v/3.24.5) (from `^3.24.3`, in `dependencies`)
