---
title: @mastra/auth-studio
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/auth/studio/CHANGELOG.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- Added `@mastra/auth-studio` â€?an auth provider for deployed Mastra Studio instances that proxies authentication through the Mastra shared API. ([#13163](https://github.com/mastra-ai/mastra/pull/13163))

  Deployed instances need no secrets â€?all WorkOS authentication is handled by the shared API. The package provides SSO login/callback flows, session management via sealed cookies, RBAC with organization-scoped permissions, and automatic forced account picker on deploy logins.
