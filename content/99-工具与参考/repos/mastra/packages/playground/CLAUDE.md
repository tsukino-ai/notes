---
title: Local Studio Standards
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/playground/CLAUDE.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- use `playground-msw-tests` skill â€?PRIMARY testing strategy for this package
- use `e2e-tests-studio` skill â€?only when MSW cannot model the journey
- use `react-best-practices` skill
- use `tailwind-best-practices` skill

## Testing Priority (highest first)

1. **Vitest + MSW + typed `@mastra/client-js` fixtures** â€?`playground-msw-tests`
...
2. **Playwright E2E** â€?`e2e-tests-studio` skill. Use only for cross-page user
...
- This package is primarily **composition** â€?prefer `playground-ui` for general UI components
