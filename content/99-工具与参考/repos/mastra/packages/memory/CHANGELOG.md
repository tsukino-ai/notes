...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/memory/CHANGELOG.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- Add `observeAttachments` to `ObservationConfig` for Observational Memory. Use it to control whether image/file parts on observed messages are forwarded to the Observer model alongside their placeholder text lines. ([#16671](https://github.com/mastra-ai/mastra/pull/16671))
  - `true` (default) â€?forward all attachments (existing behavior).
  - `false` â€?drop all attachments; placeholders still appear in the observer transcript.
  - `string[]` â€?allowlist of mimeType patterns, e.g. `['image/*']` or `['application/pdf']`. Matching is case-insensitive and supports exact, `type/*`, and `*` patterns.
...
- Fixed Observational Memory missing the observation threshold for messages with large file parts. Previously `TokenCounter`'s local/sync counting path only stringified the file descriptor (`type`, `mimeType`, `filename`) for non-image files, so a 100KB PDF looked like ~8 tokens to OM and the conversation kept replaying the full unobserved history past every reasonable threshold. ([#16562](https://github.com/mastra-ai/mastra/pull/16562))
  ...
  - Anthropic PDFs â‰?`bytes / 3` (floor 1500)
  - Google PDFs â‰?`bytes / 20` (floor 258)
  - OpenAI / unknown PDFs â‰?`bytes / 4` (floor 500)
  - Text-ish mime types (`text/*`, JSON, XML, YAML) â‰?`bytes / 4`
  - Other binary â‰?`bytes / 4`
  ...
  // counter.countMessage(message) â‰?8

  // After: estimated locally from byte size on the active provider.
  // counter.countMessage(message) â‰?tens of thousands of tokens
  //   â†?OM threshold trips as expected.
  ```
...
- Fixed reflection threshold not respecting per-record overrides set via the PATCH API. Previously, lowering the reflection threshold for a specific record had no effect on the actual reflection trigger â€?only the default 40k threshold was used. Now per-record overrides are correctly applied in both sync and async reflection paths. ([#15170](https://github.com/mastra-ai/mastra/pull/15170))
...
- **Refactored Observational Memory into modular architecture** ([#14453](https://github.com/mastra-ai/mastra/pull/14453))
  ...
  Restructured the Observational Memory (OM) engine from a single ~3,800-line monolithic class into a modular, strategy-based architecture. The public API and behavior are unchanged â€?this is a purely internal refactor that improves maintainability, testability, and separation of concerns.

  **Why** â€?The original `ObservationalMemory` class handled everything: orchestration, LLM calling, observation logic for three different scopes, reflection, buffering coordination, turn lifecycle, and message processing. This made it difficult to reason about individual behaviors, test them in isolation, or extend the system. The refactor separates these responsibilities into focused modules.

  **Observation strategies** â€?Extracted three duplicated observation code paths (~650 lines of conditionals) into pluggable strategy classes sharing a common `prepare â†?process â†?persist` lifecycle via an abstract base class. The correct strategy is selected automatically based on scope and buffering configuration.

  ```
  observation-strategies/
    base.ts            â€?abstract ObservationStrategy + StrategyDeps interface
    sync.ts            â€?SyncObservationStrategy (thread-scoped synchronous)
    async-buffer.ts    â€?AsyncBufferObservationStrategy (background buffered)
    resource-scoped.ts â€?ResourceScopedObservationStrategy (multi-thread)
    index.ts           â€?static factory: ObservationStrategy.create(om, opts)
  ```

  ```ts
  // Internal usage â€?strategies are selected and run automatically:
  ...
  **Dedicated runners** â€?Moved observer and reflector LLM-calling logic into `ObserverRunner` (194 lines) and `ReflectorRunner` (710 lines), separating prompt construction, degenerate output detection, retry logic, and compression level escalation from orchestration. `BufferingCoordinator` (175 lines) extracts the static buffering state machine and async operation tracking.

  **Processor** â€?Added `ObservationalMemoryProcessor` implementing the `Processor` interface, bridging the OM engine with the AI SDK message pipeline. It owns the decision of _when_ to buffer, activate, observe, and reflect â€?while the OM engine owns _how_ to do each operation.
  ...
  **Unified OM engine instantiation** â€?Replaced the duplicated `getOMEngine()` singleton and per-call `createOMProcessor()` engine creation with a single lazy `omEngine` property on the `Memory` class. This eliminates config drift between the legacy `getContext()` API and the processor pipeline â€?both now share the same `ObservationalMemory` instance with the full configuration.
  ...
  **Improved observation activation atomicity** â€?Added conditional WHERE clauses to `activateBufferedObservations` in all storage adapters (pg, libsql, mongodb) to prevent duplicate chunk swaps when concurrent processes attempt activation simultaneously. If chunks have already been cleared by another process, the operation returns early with zero counts instead of corrupting state.

  **Compression start level from model context** â€?Integrated model-aware compression start levels into the `ReflectorRunner`. Models like `gemini-2.5-flash` that struggle with light compression now start at compression level 2 instead of 1, reducing wasted reflection retries.

  **Pure function extraction** â€?Moved reusable helpers into `message-utils.ts`: `filterObservedMessages`, `getBufferedChunks`, `sortThreadsByOldestMessage`, `stripThreadTags`. Eliminated dead code including `isObserving` DB flag, `countMessageTokens`, `acquireObservingLock`/`releaseObservingLock`, and ~10 cascading dead private methods.

  **Cleanup** â€?Dropped `threadIdCache` (pointless memoization), removed `as any` casts for private method access (made methods properly public with `@internal` tsdoc), replaced sealed-ID-based tracking with message-level `metadata.mastra.sealed` flag checks.
...
- Fixed observational memory buffering to preserve more context and activate at the right time. ([#13476](https://github.com/mastra-ai/mastra/pull/13476))
  ...
  - **Fixed** partial activations that left too much context â€?activation is now skipped when it can't compress enough, falling back to a full observation instead.
  - **Fixed** token counting so reasoning-only message parts no longer inflate totals and cause premature context reduction.
  - **Clarified** `blockAfter` behavior: values below 100 are treated as multipliers (e.g. `1.2` = 1.2Ã— threshold), values â‰?100 as absolute token counts.
...
- Improved Observational Memory activation to preserve more usable context after activation. Previously, activation could leave the agent with too much or too little context depending on how chunks aligned with the retention target. ([#13305](https://github.com/mastra-ai/mastra/pull/13305))
  ...
  - `bufferActivation` now accepts absolute token values (>= 1000) in addition to ratios (0â€?), giving more precise control over when activation triggers
...
- dependencies updates: ([#13167](https://github.com/mastra-ai/mastra/pull/13167))
  - Updated dependency [`lru-cache@^11.2.6` â†—ï¸Ž](https://www.npmjs.com/package/lru-cache/v/11.2.6) (from `^11.2.2`, in `dependencies`)
...
- Fixed a bug where the OM context window would jump to extremely high token counts (e.g. 16k â†?114k) after observation activation. Two issues were causing this: ([#13070](https://github.com/mastra-ai/mastra/pull/13070))
  ...
  - **Marker duplication on messages**: Activation markers were being added to assistant messages twice â€?once by the AI SDK stream and once by the persistence layer â€?doubling every marker and compounding the token inflation. Both `persistMarkerToMessage` and `persistMarkerToStorage` now deduplicate by `type + cycleId` before adding a marker.
...
- @mastra/opencode: Add opencode plugin for Observational Memory integration ([#12925](https://github.com/mastra-ai/mastra/pull/12925))
  ...
  **New exports:**
  - `ObserveHooks` â€?lifecycle callbacks (`onObservationStart`, `onObservationEnd`, `onReflectionStart`, `onReflectionEnd`) for hooking into observation/reflection cycles
  - `OBSERVATION_CONTEXT_PROMPT` â€?preamble that introduces the observations block
  - `OBSERVATION_CONTEXT_INSTRUCTIONS` â€?rules for interpreting observations (placed after the `<observations>` block)
  - `OBSERVATION_CONTINUATION_HINT` â€?behavioral guidance that prevents models from awkwardly acknowledging the memory system
  - `getOrCreateRecord()` â€?now public, allows eager record initialization before the first observation cycle
...
- Added Observational Memory â€?a new memory system that keeps your agent's context window small while preserving long-term memory across conversations. ([#12599](https://github.com/mastra-ai/mastra/pull/12599))

  **Why:** Long conversations cause context rot and waste tokens. Observational Memory compresses conversation history into observations (5â€?0x compression) and periodically condenses those into reflections. Your agent stays fast and focused, even after thousands of messages.
  ...
  **What's new:**
  - `observationalMemory: true` enables the three-tier memory system (recent messages â†?observations â†?reflections)
...
- Remove various deprecated APIs from agent class. ([#9257](https://github.com/mastra-ai/mastra/pull/9257))
  - `agent.llm` â†?`agent.getLLM()`
  - `agent.tools` â†?`agent.getTools()`
  - `agent.instructions` â†?`agent.getInstructions()`
  - `agent.speak()` â†?`agent.voice.speak()`
  - `agent.getSpeakers()` â†?`agent.voice.getSpeakers()`
  - `agent.listen` â†?`agent.voice.listen()`
  - `agent.fetchMemory` â†?`(await agent.getMemory()).query()`
  - `agent.toStep` â†?Add agent directly to the step, workflows handle the transformation
...
- dependencies updates: ([#10133](https://github.com/mastra-ai/mastra/pull/10133))
  - Updated dependency [`js-tiktoken@^1.0.21` â†—ï¸Ž](https://www.npmjs.com/package/js-tiktoken/v/1.0.21) (from `^1.0.20`, in `dependencies`)
...
- dependencies updates: ([#8635](https://github.com/mastra-ai/mastra/pull/8635))
  - Updated dependency [`redis@^5.8.3` â†—ï¸Ž](https://www.npmjs.com/package/redis/v/5.8.3) (from `^5.8.2`, in `dependencies`)

- dependencies updates: ([#8684](https://github.com/mastra-ai/mastra/pull/8684))
  - Updated dependency [`@upstash/redis@^1.35.5` â†—ï¸Ž](https://www.npmjs.com/package/@upstash/redis/v/1.35.5) (from `^1.35.4`, in `dependencies`)
...
- dependencies updates: ([#8088](https://github.com/mastra-ai/mastra/pull/8088))
  - Updated dependency [`@upstash/redis@^1.35.4` â†—ï¸Ž](https://www.npmjs.com/package/@upstash/redis/v/1.35.4) (from `^1.35.3`, in `dependencies`)
...
- ab48c97: dependencies updates:
  - Updated dependency [`zod-to-json-schema@^3.24.6` â†—ï¸Ž](https://www.npmjs.com/package/zod-to-json-schema/v/3.24.6) (from `^3.24.5`, in `dependencies`)
...
- [#6919](https://github.com/mastra-ai/mastra/pull/6919) [`6e7e120`](https://github.com/mastra-ai/mastra/commit/6e7e1207d6e8d8b838f9024f90bd10df1181ba27) Thanks [@dane-ai-mastra](https://github.com/apps/dane-ai-mastra)! - dependencies updates:
  - Updated dependency [`ai@^4.3.19` â†—ï¸Ž](https://www.npmjs.com/package/ai/v/4.3.19) (from `^4.3.16`, in `dependencies`)
  - Updated dependency [`ai-v5@npm:ai@5.0.15` â†—ï¸Ž](https://www.npmjs.com/package/ai-v5/v/5.0.15) (from `npm:ai@5.0.0`, in `dependencies`)
...
- [#6921](https://github.com/mastra-ai/mastra/pull/6921) [`c4f2605`](https://github.com/mastra-ai/mastra/commit/c4f2605fa00e796a26c6f5fc3cf8f5552f77d617) Thanks [@dane-ai-mastra](https://github.com/apps/dane-ai-mastra)! - dependencies updates:
  - Updated dependency [`redis@^5.8.2` â†—ï¸Ž](https://www.npmjs.com/package/redis/v/5.8.2) (from `^5.8.1`, in `dependencies`)
...
- 8a41506: dependencies updates:
  - Updated dependency [`redis@^5.8.1` â†—ï¸Ž](https://www.npmjs.com/package/redis/v/5.8.1) (from `^5.8.0`, in `dependencies`)
...
- 7d53af4: dependencies updates:
  - Updated dependency [`redis@^5.7.0` â†—ï¸Ž](https://www.npmjs.com/package/redis/v/5.7.0) (from `^5.6.0`, in `dependencies`)
- e1a2ef1: dependencies updates:
  - Updated dependency [`@upstash/redis@^1.35.3` â†—ï¸Ž](https://www.npmjs.com/package/@upstash/redis/v/1.35.3) (from `^1.35.1`, in `dependencies`)
- f082460: dependencies updates:
  - Updated dependency [`redis@^5.8.0` â†—ï¸Ž](https://www.npmjs.com/package/redis/v/5.8.0) (from `^5.7.0`, in `dependencies`)
...
- 674a348: dependencies updates:
  - Updated dependency [`@upstash/redis@^1.35.1` â†—ï¸Ž](https://www.npmjs.com/package/@upstash/redis/v/1.35.1) (from `^1.35.0`, in `dependencies`)
...
- 138f4a2: dependencies updates:
  - Updated dependency [`redis@^5.6.0` â†—ï¸Ž](https://www.npmjs.com/package/redis/v/5.6.0) (from `^4.7.1`, in `dependencies`)
...
- fb2a1b9: dependencies updates:
  - Updated dependency [`pg@^8.16.3` â†—ï¸Ž](https://www.npmjs.com/package/pg/v/8.16.3) (from `^8.16.0`, in `dependencies`)
- bc2bbdc: dependencies updates:
  - Updated dependency [`pg-pool@^3.10.1` â†—ï¸Ž](https://www.npmjs.com/package/pg-pool/v/3.10.1) (from `^3.10.0`, in `dependencies`)
...
- 4051477: dependencies updates:
  - Added dependency [`zod-to-json-schema@^3.24.5` â†—ï¸Ž](https://www.npmjs.com/package/zod-to-json-schema/v/3.24.5) (to `dependencies`)
- 63f6b7d: dependencies updates:
  - Updated dependency [`@upstash/redis@^1.35.0` â†—ï¸Ž](https://www.npmjs.com/package/@upstash/redis/v/1.35.0) (from `^1.34.5`, in `dependencies`)
  - Updated dependency [`ai@^4.3.16` â†—ï¸Ž](https://www.npmjs.com/package/ai/v/4.3.16) (from `^4.2.2`, in `dependencies`)
  - Updated dependency [`js-tiktoken@^1.0.20` â†—ï¸Ž](https://www.npmjs.com/package/js-tiktoken/v/1.0.20) (from `^1.0.19`, in `dependencies`)
  - Updated dependency [`pg@^8.16.0` â†—ï¸Ž](https://www.npmjs.com/package/pg/v/8.16.0) (from `^8.13.3`, in `dependencies`)
  - Updated dependency [`pg-pool@^3.10.0` â†—ï¸Ž](https://www.npmjs.com/package/pg-pool/v/3.10.0) (from `^3.7.1`, in `dependencies`)
  - Updated dependency [`postgres@^3.4.7` â†—ï¸Ž](https://www.npmjs.com/package/postgres/v/3.4.7) (from `^3.4.5`, in `dependencies`)
  - Updated dependency [`redis@^4.7.1` â†—ï¸Ž](https://www.npmjs.com/package/redis/v/4.7.1) (from `^4.7.0`, in `dependencies`)
