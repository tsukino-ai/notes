---
title: Observational Memory: Production Readiness Report
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/memory/src/processors/observational-memory/explorations/prod-readiness.md
tags:
  - agent-framework
  - typescript
  - mastra
...
The most severe issue is **data loss in resource-scoped OM** â€?concurrent observations from different replicas overwrite each other's results due to read-modify-write patterns without compare-and-swap or versioning.
...
| `lastBufferedAtTime`       | Timestamp cursor for which messages have been buffered | Same â€?Replica B re-buffers already-buffered messages.                                                                           |
| `reflectionBufferCycleIds` | CycleId for in-flight buffered reflections             | Minor â€?affects UI marker display only.                                                                                          |
...
`operation-registry.ts` uses a static `Map<string, number>` to track active operations (observing, reflecting, buffering). Process-local only. The stale flag detection pattern â€?"DB flag says observing is active, but `isOpActiveInProcess()` returns false, so the flag must be stale" â€?works as partial crash recovery within a single process but cannot coordinate across replicas.
...
`updateActiveObservations` is a plain overwrite â€?no version checking, no CAS, no optimistic concurrency control.
...
### Scenario 1: Resource-Scoped OM â€?Two Threads, Same Resource, Different Replicas
...
1. Replica A observes thread-1 messages â†?generates `obs1`, writes `observedMessageIds: [msg1, msg2]`
2. Replica B observes thread-2 messages â†?generates `obs2`, writes `observedMessageIds: [msg3, msg4]`
3. Last write wins: `obs1` is gone, `[msg1, msg2]` are gone from the safeguard set

This is not theoretical â€?any two agents sharing a `resourceId` that happen to land on different servers will hit this.
...
4. Replica B writes back `record v2'` with `obs2` â€?**overwrites `obs1`**
...
3. Replica B's `asyncBufferingOps` map is empty â€?it can't await Replica A's in-flight operation
...
6. When Replica A's async op eventually completes (if the process is still alive), it writes additional buffered chunks that have already been "activated" â€?creating orphaned state
...
3. Lambda B starts cold â€?all static maps are empty
4. Lambda B reads DB: `isBufferingObservation = true` (stale flag from Lambda A)
5. Lambda B calls `isOpActiveInProcess()` â†?returns `false` (no op in this process)
...
| -------------------------------------------------------- | ------------ | ------------------------------------------------------------------- |
| Resource-scoped observation RMW (observations lost)      | **Critical** | Actual data loss â€?observations from one thread overwrite another's |
...
1. **Compare-and-swap or optimistic concurrency** on `updateActiveObservations` â€?reject writes if the record version has changed
...
4. **Idempotent activation** â€?`swapBufferedToActive` should be safe to call twice with the same chunks
5. **Atomic flag transitions** â€?`isReflecting` and `isBufferingObservation` flags should use conditional updates (e.g., `UPDATE ... SET isReflecting = true WHERE isReflecting = false`)
