---
title: OM Repro Capture Workflow
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/memory/src/processors/observational-memory/__fixtures__/repro-captures/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- `input.json` â€?process input metadata (step/readOnly/state keys + JSON-safe `state` + captured replay `args`)
- `pre-state.json` â€?OM state before processing:
  - OM record snapshot
  - buffered chunks
  - `contextTokenCount`
  - raw `messages`
  - serialized `messageList`
- `output.json` â€?process output details:
...
- `post-state.json` â€?same shape as pre-state after processing
