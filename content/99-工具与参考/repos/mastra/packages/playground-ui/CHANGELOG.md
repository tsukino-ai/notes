---
title: @mastra/playground-ui
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/playground-ui/CHANGELOG.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- Pointer drags inside the `SideDialog` body now select text reliably instead of fighting with the close-swipe gesture. The popup chrome (header, edges) still closes the drawer on drag. ([#16959](https://github.com/mastra-ai/mastra/pull/16959))
  ...
  **Drawer composition**
  ...
  ```tsx
  <Drawer>
    <DrawerTrigger>â€?/DrawerTrigger>
    <DrawerContent>
      <DrawerHeader>â€?/DrawerHeader>
      <DrawerBody>â€?/DrawerBody>
    </DrawerContent>
  </Drawer>
  ```
  ...
  Base UI's text-selectable region (the `Drawer.Content` part â€?pointer drags inside it select text instead of closing the drawer) is now exported as `DrawerInteractive`. Migration:
...
- Removed the EntityList and EntityListPageLayout components â€?they were superseded by DataList. The 9 Studio root list pages (Agents, Datasets, Experiments, MCPs, Processors, Prompts, Scorers, Tools, Workflows) now build on DataList, matching the condensed layout shared across the rest of Studio. ([#17058](https://github.com/mastra-ai/mastra/pull/17058))

  **Migration**

  If you were importing EntityList or EntityListPageLayout from `@mastra/playground-ui`, switch to DataList â€?its API is a strict superset:
  ...
  The primitive names match (`.Top`, `.TopCell`, `.TopCellSmart`, `.RowLink`, `.Cell`, `.TextCell`, `.NameCell`, `.DescriptionCell`, `.NoMatch`, `.Pagination`). DataList additionally exposes `.Row`, `.RowButton`, `.RowStatic`, `.IdCell`, `.MonoCell`, `.SelectCell`, `.NextPageLoading`, and more â€?useful when you need selection rows or action cells outside a RowLink.
...
- Added a Drawer component â€?a panel that slides in from any edge of the screen with swipe-to-dismiss gestures. ([#16958](https://github.com/mastra-ai/mastra/pull/16958))
...
- Migrated the Switch component to Base UI for smoother animations and consistent behavior. No API changes â€?`checked`, `defaultChecked`, `onCheckedChange`, and `disabled` work exactly as before. ([#16891](https://github.com/mastra-ai/mastra/pull/16891))
...
- Added `DataList.RowStatic`, a non-interactive row primitive. It renders a row that looks identical to other list rows but does not respond to clicks and shows no hover/focus state â€?use it alongside `DataList.RowButton` / `DataList.RowLink` when only some rows are clickable (e.g. error or placeholder entries in an otherwise navigable list). ([#16970](https://github.com/mastra-ai/mastra/pull/16970))
...
- Added `DataList` primitives and props for building selection-aware, condensed list rows that match the Traces/Logs visual style. ([#16820](https://github.com/mastra-ai/mastra/pull/16820))
  ...
  **New cells** on `DataList`:
  - `IdCell` â€?compact mono cell that truncates long IDs to 8 chars.
  - `MonoCell` â€?compact mono + truncate text cell (for input previews, JSON summaries, etc.).
  - `DateCell` â€?compact date cell rendering "Today" or "MMM dd".
  - `TimeCell` â€?compact mono time cell rendering `HH:mm:ss.SSS` with the millisecond portion tinted.
  - `SelectCell` â€?labelled checkbox cell with a shift-key range-select handler.
  - `TopSelectCell` â€?header version with `indeterminate` support for "select all".
  - `TopCells` â€?non-interactive header sibling of `RowButton`, for hosting top cells beside a leading select cell.

  **New props** on `DataList.RowButton` and `DataList.RowLink`:
  - `flushLeft` â€?drops the default left margin when wrapped beside a leading cell.
  - `colStart` â€?places the row starting at a column line (e.g. `colStart={2}` to leave column 1 for a leading cell).
  - `featured` â€?applies the highlighted background to mark the active row.

  **New props** on existing wrappers:
  - `as` on `DataList.Cell` and `DataList.TopCell` â€?render the cell as any HTML element (e.g. `<label>` so the whole cell is clickable).
  - `hasLeadingCell` on `DataList.Top` â€?drops default gap and left padding so a leading cell sits flush, mirroring how `Row` + `RowButton` compose.

  **Example** â€?selection row with a checkbox in column 1 and an interactive button spanning the rest:
  ...
  Internally the Traces and Logs list views now use the shared primitives â€?no behavior change for those consumers.
...
- Added support for trailing cells in `DataList` rows. `DataList.RowButton` and `DataList.RowLink` now accept `colEnd` and `flushRight` (mirrors of the existing `colStart`/`flushLeft`), so a row can sit beside a non-interactive trailing cell (e.g. an actions column) and stay aligned with the header. Rows wrapped in `DataList.Row` now render a full-width separator that extends through the leading and trailing cells. `DataList.MonoCell` also gained an optional `height` prop so non-compact lists can use it without forcing compact padding. ([#16888](https://github.com/mastra-ai/mastra/pull/16888))
  ...
  **Usage**
  ...
  <DataList.MonoCell height="default">long mono textâ€?/DataList.MonoCell>
  ```

- Migrated the Label component off Radix UI. It now renders a native `<label>` element with the same props and styling â€?`htmlFor`, `className`, and children behave exactly as before. ([#16892](https://github.com/mastra-ai/mastra/pull/16892))
...
- Restyled scrollbars across the studio UI to match the design system â€?thin, themed thumb on a transparent track â€?replacing the default OS scrollbars that clashed with dark and light surfaces. ([#16918](https://github.com/mastra-ai/mastra/pull/16918))
  ...
  **Migration:**
  ...
  <EntryList>
    <EntryList.Trim>
      ...
      <EntryList.Entries>
        {items.map(item => (
          <EntryList.Entry key={item.id} columns={columns} entry={item} onClick={â€¦}>
            {columns.map(col => <EntryList.EntryText key={col.name}>{item[col.name]}</EntryList.EntryText>)}
          </EntryList.Entry>
        ))}
      </EntryList.Entries>
    </EntryList.Trim>
    <EntryList.Pagination â€?>
  </EntryList>
  ...
  <DataList columns={gridColumns}>
    ...
    {items.map(item => (
      <DataList.RowButton key={item.id} onClick={â€¦}>
        {columns.map(col => <DataList.Cell key={col.name}>{item[col.name]}</DataList.Cell>)}
      </DataList.RowButton>
    ))}
    <DataList.Pagination â€?>
  </DataList>
  ```

- Improved the Checkbox component by migrating it to Base UI. The public API is unchanged â€?`checked` (including the `'indeterminate'` value), `defaultChecked`, `onCheckedChange`, and `disabled` all behave as before. ([#16905](https://github.com/mastra-ai/mastra/pull/16905))
...
- Improved the RadioGroup component by migrating it to Base UI. The public API (`RadioGroup`, `RadioGroupItem`, `value`, `onValueChange`, `disabled`) is unchanged. Also fixes the radio indicator sizing/centering â€?the control now stays square and the inner dot is properly centered. ([#16904](https://github.com/mastra-ai/mastra/pull/16904))
...
- Added `ContextMenu` for right-click interactions. Supports submenus, checkbox and radio items, keyboard shortcuts, and a `destructive` variant for dangerous actions like delete. ([#16791](https://github.com/mastra-ai/mastra/pull/16791))
  ...
  <ContextMenu>
    <ContextMenu.Trigger className="â€?>Right click here</ContextMenu.Trigger>
    <ContextMenu.Content>
...
- Refreshed Button + Card design system tokens. ([#16769](https://github.com/mastra-ai/mastra/pull/16769))
  ...
  **Button variants (breaking)**: consolidated to `default`, `primary`, `outline`, `ghost`. The `cta`, `contrast`, and unused `link` variants have been removed. `primary` now uses a high-contrast `neutral6` fill instead of `surface4`, so it reads clearly as the form submit action in both themes.
  ...
  // After
  <Button variant="primary">Save</Button>     // cta â†?primary (no brand green; theme-aware high contrast)
  <Button variant="primary">Done</Button>     // contrast â†?primary (same recipe, renamed)
  <Button as="a" href="â€? variant="ghost">Open</Button>  // link â†?ghost (or plain <a> for inline text links)
  ```

  **New tokens**: `--surface-overlay-soft` and `--surface-overlay-strong` â€?alpha overlays of the opposite-theme color, used by `SectionCard` header strip and `DashboardCard` fill so cards read consistently on any surface.

  **Other**:
  - DashboardCard radius reduced to `rounded-xl` and padding tightened to `px-4 py-3` for better grid density.
  - SectionCard wrapper no longer fills its background â€?header strip + border carry definition.
  - Dark `surface2` / `surface3` darkened slightly (16.84% â†?16%, 19.13% â†?18%) so the main frame reads as a distinct surface.
  - Dark `border1` / `border2` alphas bumped (6% â†?7%, 10% â†?11%) for closer dark/light parity.
...
- `AlertDialog`'s API and behavior are unchanged â€?`asChild` on `AlertDialog.Trigger`, and `AlertDialog.Action` and `AlertDialog.Cancel`, all work exactly as before. (Internally it now builds on Base UI primitives.) ([#16824](https://github.com/mastra-ai/mastra/pull/16824))

- Moved the `Collapsible` component to Base UI, with a smoother height-based expand and collapse animation. The public API is unchanged â€?`asChild` on `CollapsibleTrigger` still works. ([#16825](https://github.com/mastra-ai/mastra/pull/16825))

- Upgraded `@base-ui/react` to 1.5, making popups noticeably faster â€?components built on Base UI such as `Tooltip`, `Popover`, `DropdownMenu` and `ContextMenu` now open and close more quickly. ([#16819](https://github.com/mastra-ai/mastra/pull/16819))
...
- The Traces list now updates live via delta polling. Previously the list was refetched every 10 seconds, replacing the whole page with no signal about what changed; now new traces appear within a few seconds of being created, with a brief highlight to draw attention. Status changes on already-visible rows (running â†?success / error) also propagate without intervention, and returning to the tab after being idle re-syncs from a fresh cursor. ([#16727](https://github.com/mastra-ai/mastra/pull/16727))

  **New `useTraces` return fields**
  - `isRefetching` â€?true while any meaningful refetch is in flight. Use it to drive a heartbeat indicator.
  - `autoRefetch` / `setAutoRefetch` â€?pause / resume all automatic polling so the consumer can render an opt-out toggle.
  - `recentlyAddedKeys` â€?`Set<string>` of `traceId:spanId` for rows that just arrived via delta polling. Drives the temporary highlight in `TracesListView`.
  ...
  **TracesListView**
  ...
  New optional `recentlyAddedKeys?: Set<string>` prop. Rows whose `traceId:spanId` is in the set get the `animate-row-highlight` class â€?a brief fade-out to transparent, added to `index.css`.

  **Compatibility**

  Requires `@mastra/server` and `@mastra/client-js` at the versions that ship the observability delta-polling endpoints, and a store that opts into delta polling (`@mastra/clickhouse`, `@mastra/duckdb`, and the in-memory store today). When unavailable â€?older server or a store without delta capability â€?the hook silently falls back to page-mode interval refetching. No consumer changes required.
...
- Moved the `Dialog` component to Base UI. The public API is unchanged â€?`asChild` on `DialogTrigger` and `DialogClose` still works the same way, and open/close animations behave as before. ([#16821](https://github.com/mastra-ai/mastra/pull/16821))
...
- Improved the Observability traces list to make the with-subtraces view more discoverable. ([#16643](https://github.com/mastra-ai/mastra/pull/16643))
  ...
  - **Added:** A standalone **Show subtraces** toggle next to **Add Filter** â€?off keeps the default top-level view, on includes subtraces.
  - **Removed:** The **List mode** entry from the **Add Filter** menu (now driven by the toggle).

  **Usage:** Open Observability â†?Traces â†?switch **Show subtraces** on. The **Level** column updates: top-level rows keep the Trace icon, nested rows show the Subtrace (â†? icon. The toggle is hidden automatically when the active storage provider doesn't support subtraces.

- Removed the "Group traces by thread" option from the Observability traces page. The list now always displays a flat view of traces, without thread-id subheaders. To narrow results to a specific thread, use the Thread ID property in the Add filter menu (open Observability â†?Traces â†?Add filter â†?Thread ID â†?paste the threadId). ([#16633](https://github.com/mastra-ai/mastra/pull/16633))
...
- Restyled `MainSidebar` and swapped sidebar icons to the new Figma design system set. ([#16544](https://github.com/mastra-ai/mastra/pull/16544))
  - Section titles are larger and medium-weight (`text-ui-sm`, `font-medium`), lowercase, muted â€?replacing the previous uppercase + wide-tracking treatment. Underline divider beneath the title removed in both expanded and collapsed states. Active indicator bar on the left edge removed.
...
- **Updated agent traces tab to use the rich observability traces UI** ([#16405](https://github.com/mastra-ai/mastra/pull/16405))
  ...
  **Locked scope filter pills**
  ...
  When viewing agent-scoped traces, the Primitive Type and Primitive ID filter pills are now read-only â€?they display the agent context, show a lock icon and tooltip, and cannot be edited or removed. The Add Filter dropdown no longer lists scope-controlled fields so users cannot accidentally override the active scope.
  ...
  // After â€?pills for the listed fields render locked with a tooltip
...
- Improved `ScrollArea` to use Base UI internally and added a richer mask API. Edges now fade by default based on `orientation` (top/bottom for vertical, left/right for horizontal, all four for both), so most scrollers get the polished fade-out automatically. ([#16415](https://github.com/mastra-ai/mastra/pull/16415))
  ...
  **Heads up â€?default behavior change:** `ScrollArea` previously rendered without any edge fade unless `showMask` was passed. It now fades the edges that match `orientation` by default. Pass `mask={false}` on the callsites where you want to keep the old hard edges.
  ...
  **New `mask` prop.** Accepts a boolean (`false` disables the fade entirely) or an object to override individual sides. The `x` and `y` keys are shorthands for the matching axis.
  ...
  ```tsx
  // Default â€?fades follow `orientation`
  ...
  **Migrating from `showMask`.** The `showMask` boolean is now deprecated but still works â€?`mask` wins when both are set.
...
- Added an "All traces, nested too" mode to the Observability â†?Traces page. ([#16479](https://github.com/mastra-ai/mastra/pull/16479))

  The traces list now has a switcher in the toolbar to toggle between two views:
  - **Top-level traces only** (default) â€?one row per top-level run, the existing behavior.
  - **All traces, nested too** â€?one row per invocation, including every agent, workflow, tool, processor, scorer, and RAG ingestion that ran nested inside another run.
  ...
  **New hooks** for consumers building their own observability UIs:
  - `useBranch({ traceId, spanId, depth? })` â€?fetches the span subtree rooted at an anchor span.
  - `useTraceOrBranchSpans({ traceId, spanId, listMode })` â€?returns trace spans or a branch subtree depending on the active mode.
...
- Added a new `pill` variant on `TabList` with an animated background indicator that slides behind the active trigger. The default `line` variant now animates its underline smoothly between tabs as well. Implemented by migrating the underlying Tabs component from Radix UI to Base UI. ([#16414](https://github.com/mastra-ai/mastra/pull/16414))
  ...
  ```tsx
  // Before â€?only the line (underline) style was available
  ...
  // After â€?opt into the new pill style via the `variant` prop on TabList
  <Tabs defaultTab="overview">
...
- **Added** new `pill-ghost` variant on `Tabs` and `sticky` prop on `TabList` for sticky tab headers. ([#16433](https://github.com/mastra-ai/mastra/pull/16433))
  ...
  **Improved** `EntityHeader` layout â€?title and children now share a single row with wrapping, and padding is tighter for denser headers.
...
- Added `InputGroup` and extended `ButtonsGroup` in playground-ui design system. ([#16417](https://github.com/mastra-ai/mastra/pull/16417))
  ...
  **Extended `ButtonsGroup`**
  ...
  <ButtonsGroup spacing="close">
    <Button variant="outline">âˆ?/Button>
  ...
  **Tweaked `Button` ghost variant**
  ...
  Aligned hover/active progression with the outline variant (`surface3` â†?`surface4`) so click feedback is perceptible on transparent backgrounds. Existing ghost buttons throughout the playground will appear one shade lighter on hover and active.
...
- Filter pills (`PropertyFilterApplied`) now match the rest of the design system â€?single 1px border, consistent rounded segments, no custom styling. Labels stay on one line and no longer compress when long values are present. ([#16426](https://github.com/mastra-ai/mastra/pull/16426))
...
- Fixed three issues on the Logs and Traces pages: ([#16306](https://github.com/mastra-ai/mastra/pull/16306))
  - Column widths now stay stable while scrolling â€?they no longer jump as different rows scroll into view.
...
- Refreshed the visual style of form controls and popups for a softer, more consistent look: ([#16150](https://github.com/mastra-ai/mastra/pull/16150))
  ...
  - **Tokens:** bumped the radius scale (`sm` 2â†?px, `md` 4â†?px, `lg` 6â†?0px, `xl` 12â†?4px); replaced `--shadow-dialog`'s outer 1px ring with an inset top gloss so the dialog shadow stops doubling up with each consumer's own border.
...
- **Polished Combobox dropdown items** ([#16411](https://github.com/mastra-ai/mastra/pull/16411))
  ...
  **Added `ComboboxPrimitive` export for advanced compositions**
  ...
  Re-exports the raw `@base-ui/react/combobox` parts (Root, Trigger, Input, List, Item, Chips, etc.) so callers needing virtualization, async status, chips, or creatable patterns can compose them directly with the shared `comboboxStyles` tokens â€?without growing the monolithic `<Combobox>` prop surface.
...
- Changed the default Observability list mode to branches (all traces, including nested). The query logic still recognizes `?listMode=traces` to opt back into the top-level-only view. ([#16493](https://github.com/mastra-ai/mastra/pull/16493))
  ...
  **Before**
  ...
  `/observability` â†?top-level traces only

  **After**

  `/observability` â†?branches (all traces, nested too)
  `/observability?listMode=traces` â†?top-level traces only
...
- Added support for icon-and-description layout in `Notice` by making `title` optional. When omitted, the notice renders as a single row with icon and description, useful for inline contextual messages. ([#16033](https://github.com/mastra-ai/mastra/pull/16033))
  ...
  ```tsx
  // Before â€?title required
  <Notice variant="info" title="Heads up">Some message.</Notice>

  // After â€?title optional, single-row layout
...
- Refreshed toast styling so it aligns with the Notice component and lets sonner own the layout. ([#16144](https://github.com/mastra-ai/mastra/pull/16144))
  ...
  **What changed for users:**
  ...
  - Sonner's native layout is back in charge â€?the loader on `toast.promise`, the close button position, the icon placement and the mobile width all work as documented instead of fighting custom overrides.
...
- Added MCP Apps extension support (SEP-1865). MCPServer now accepts an `appResources` config to register interactive `ui://` HTML resources. MCPClient preserves full tool `_meta` (including `ui.resourceUri`) when converting MCP tools to Mastra tools. Both advertise the `io.modelcontextprotocol/ui` extension capability. ([#16004](https://github.com/mastra-ai/mastra/pull/16004))
  ...
  **Example â€?MCPServer with app resources:**
...
- Refactored Button component to use a single `cva` (class-variance-authority) variant config instead of nested manual maps. Consolidated `IconButton` into `Button` via `size="icon-sm|icon-md|icon-lg"` and removed the `IconButton` export. Replaced `variant="light"` and `variant="inputLike"` with `variant="default"` (no behavior change for default styling). Added `cta` and `outline` variants and unified active/hover styles between text- and icon-mode buttons. ([#15985](https://github.com/mastra-ai/mastra/pull/15985))
  ...
  **Why:** A single source of truth for variants means consistent visuals, fewer drift bugs, simpler maintenance, and a more predictable surface for AI agents â€?single-variant cva is the dominant shadcn pattern across DS components in this repo (`Card`, `Input`, `Label`, `Textarea`, `StatusBadge`).
  ...
  **Migration:**
  ...
  <Button variant="default">â€?/Button>
...
- Migrated color tokens to oklch() for perceptually uniform, wide-gamut (P3) ready colors. Light theme neutrals and surfaces no longer have a blue tint (slate â†?true gray). Dark theme is visually unchanged. ([#15713](https://github.com/mastra-ai/mastra/pull/15713))
...
- Added `ErrorBoundary` component to catch and display runtime errors in the studio. Wraps routes in the local playground so a crash on one page (e.g. an agent editor referencing an unresolved workspace skill) surfaces a friendly recovery UI with **Try again** (in-place React reset), **Reload page** (full browser refresh), and **Report issue** (opens the Mastra GitHub issues page in a new tab) actions, plus a collapsible stack trace â€?instead of a blank screen. ([#15561](https://github.com/mastra-ai/mastra/pull/15561))
  ...
  **Usage**
  ...
  Props: `fallback` (node or render prop with `{ error, errorInfo, reset }`), `onError` for reporting, `resetKeys` for automatic reset, `variant` (`'section'` â€?fills available space, default; `'inline'` â€?stays compact), and `title` / `description` overrides.
...
- dependencies updates: ([#15525](https://github.com/mastra-ai/mastra/pull/15525))
  - Updated dependency [`prettier@^3.8.3` â†—ï¸Ž](https://www.npmjs.com/package/prettier/v/3.8.3) (from `^3.8.1`, in `dependencies`)
...
- Align BrandLoader geometry with the Mastra logo: match disk positions to the logo path, introduce per-size stroke widths and bubble radii (sm/md/lg), and rebalance the gooey filter for rounder ridgeâ†”disk fillets. Shift the size scale so sm stays, md is now w-8, lg is now w-10, and the old w-16 size is removed. ([#15531](https://github.com/mastra-ai/mastra/pull/15531))
...
- Improved visual consistency across Chip, DropdownMenu, Notification, Popover, and toast components â€?unified radius and border scale. Deduplicated dropdown menu item classes and added max-height scroll handling for long menus. ([#15440](https://github.com/mastra-ai/mastra/pull/15440))
...
- dependencies updates: ([#15085](https://github.com/mastra-ai/mastra/pull/15085))
  - Updated dependency [`@assistant-ui/react@^0.12.22` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react/v/0.12.22) (from `^0.12.21`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-markdown@^0.12.8` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-markdown/v/0.12.8) (from `^0.12.7`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-syntax-highlighter@^0.12.8` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-syntax-highlighter/v/0.12.8) (from `^0.12.7`, in `dependencies`)
...
- Added `PageLayout` and `PageHeader` compound components for consistent page structure across Mastra Studio, plus a `NoDataPageLayout` helper for 401/403/empty/error states. ([#15243](https://github.com/mastra-ai/mastra/pull/15243))
  ...
  List components (`AgentsList`, `WorkflowsList`, `ToolsList`, `ProcessorsList`, `McpServersList`, `PromptsList`, `LogsList`, `ObservabilityTracesList`) no longer handle errors or empty states internally â€?handle those at the page level. If you consume these components directly, move error/empty-state rendering to the parent.
...
- Added DataKeysAndValues component â€?a compound component for displaying key-value pairs in a grid layout with support for single or two-column modes and section headers ([#15126](https://github.com/mastra-ai/mastra/pull/15126))

- Added DateTimeRangePicker component â€?a date range selector with preset options (last 24h, 7d, 30d, etc.) and a custom range mode with dual calendar and time pickers ([#15128](https://github.com/mastra-ai/mastra/pull/15128))

- Added DataCodeSection component â€?a read-only code viewer with JSON syntax highlighting, search, multiline toggle, and an expandable fullscreen dialog ([#15125](https://github.com/mastra-ai/mastra/pull/15125))

- Added DataPanel compound component â€?a container for detail panels with header, navigation, close button, loading, and empty states ([#15127](https://github.com/mastra-ai/mastra/pull/15127))
...
- **New components for the Logs MVP** ([#14899](https://github.com/mastra-ai/mastra/pull/14899))
  - `DataDetailsPanel` â€?composable detail panel with header, key-value list, code sections, and loading/empty states.
  - `DataList` â€?generic data list with row links, top cells, skeleton loading, and pagination.
  - `LogsDataList` â€?log-specific data list cells (level, date, time, entity, message).
  - `SelectDataFilter` â€?dropdown filter with searchable categories, single/multi selection, and active count badge.
...
- Finished light mode support for Mastra Studio. Theme selector is now always visible in settings â€?no environment variable needed. CodeMirror editors (instructions, trace view, code diff) render with proper syntax highlighting and cursor visibility in both light and dark modes. Dropdown menus now have correct hover/focus states in light mode. ([#14796](https://github.com/mastra-ai/mastra/pull/14796))

### Patch Changes

- dependencies updates: ([#14584](https://github.com/mastra-ai/mastra/pull/14584))
  - Updated dependency [`@assistant-ui/react@^0.12.19` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react/v/0.12.19) (from `^0.12.17`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-markdown@^0.12.6` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-markdown/v/0.12.6) (from `^0.12.5`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-syntax-highlighter@^0.12.7` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-syntax-highlighter/v/0.12.7) (from `^0.12.6`, in `dependencies`)

- dependencies updates: ([#14826](https://github.com/mastra-ai/mastra/pull/14826))
  - Updated dependency [`@assistant-ui/react@^0.12.21` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react/v/0.12.21) (from `^0.12.19`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-markdown@^0.12.7` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-markdown/v/0.12.7) (from `^0.12.6`, in `dependencies`)

- dependencies updates: ([#14827](https://github.com/mastra-ai/mastra/pull/14827))
  - Updated dependency [`@uiw/codemirror-theme-dracula@^4.25.9` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/codemirror-theme-dracula/v/4.25.9) (from `^4.25.8`, in `dependencies`)
  - Updated dependency [`@uiw/codemirror-theme-github@^4.25.9` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/codemirror-theme-github/v/4.25.9) (from `^4.25.8`, in `dependencies`)
  - Updated dependency [`@uiw/react-codemirror@^4.25.9` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/react-codemirror/v/4.25.9) (from `^4.25.8`, in `dependencies`)
...
- Added Evaluate tab to the agent playground with full dataset management, scorer editing, experiment execution, and review workflow. ([#14470](https://github.com/mastra-ai/mastra/pull/14470))
  ...
  **Evaluate tab** â€?A new sidebar-driven tab for managing datasets, scorers, and experiments within the agent playground. Key features:
  ...
  **Review tab** â€?A dedicated review workflow for experiment results:
...
- Added storage type detection to the Metrics Dashboard. The `/system/packages` endpoint now returns `observabilityStorageType`, identifying the observability storage backend. The dashboard shows an empty state when the storage does not support metrics (e.g. PostgreSQL, LibSQL), and displays a warning when using in-memory storage since metrics are not persisted across server restarts. Also added a docs link button to the Metrics page header. ([#14620](https://github.com/mastra-ai/mastra/pull/14620))
  ...
  // - 'ObservabilityInMemory' â†?metrics work but are not persisted across restarts
  // - 'ObservabilityPG', 'ObservabilityLibSQL', etc. â†?metrics not supported

  if (system.observabilityStorageType === 'ObservabilityInMemory') {
    console.warn('Metrics are not persisted â€?data will be lost on server restart.');
  }
...
- dependencies updates: ([#14125](https://github.com/mastra-ai/mastra/pull/14125))
  - Updated dependency [`zustand@^5.0.11` â†—ï¸Ž](https://www.npmjs.com/package/zustand/v/5.0.11) (from `^5.0.9`, in `dependencies`)
...
- Added **Playground** and **Traces** tabs to the agent detail page. ([#13938](https://github.com/mastra-ai/mastra/pull/13938))
  ...
  **Agent Playground tab** provides a side-by-side environment for iterating on agent configuration (instructions, tools, model settings) and testing changes in a live chat â€?without modifying the deployed agent. Includes version comparison, request context configuration, and the ability to trigger dataset experiments directly from the playground.
...
- dependencies updates: ([#13864](https://github.com/mastra-ai/mastra/pull/13864))
  - Updated dependency [`@uiw/codemirror-theme-dracula@^4.25.7` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/codemirror-theme-dracula/v/4.25.7) (from `^4.25.5`, in `dependencies`)

- dependencies updates: ([#14057](https://github.com/mastra-ai/mastra/pull/14057))
  - Updated dependency [`@uiw/codemirror-theme-dracula@^4.25.8` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/codemirror-theme-dracula/v/4.25.8) (from `^4.25.7`, in `dependencies`)

- dependencies updates: ([#14062](https://github.com/mastra-ai/mastra/pull/14062))
  - Updated dependency [`prettier@^3.8.1` â†—ï¸Ž](https://www.npmjs.com/package/prettier/v/3.8.1) (from `^3.6.2`, in `dependencies`)
...
- Added support for editing code-defined agents in the Playground CMS. When a code agent is opened in the editor, only **Instructions**, **Tools**, and **Variables** sections are shown â€?model, memory, workspace, and other code-defined fields are hidden since they cannot be safely serialized. ([#13727](https://github.com/mastra-ai/mastra/pull/13727))
...
- dependencies updates: ([#13236](https://github.com/mastra-ai/mastra/pull/13236))
  - Updated dependency [`papaparse@^5.5.3` â†—ï¸Ž](https://www.npmjs.com/package/papaparse/v/5.5.3) (from `^5.4.1`, in `dependencies`)

- dependencies updates: ([#13271](https://github.com/mastra-ai/mastra/pull/13271))
  - Updated dependency [`@codemirror/merge@^6.12.0` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/merge/v/6.12.0) (from `^6.10.2`, in `dependencies`)
  - Updated dependency [`@codemirror/view@^6.39.15` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/view/v/6.39.15) (from `^6.39.14`, in `dependencies`)

- dependencies updates: ([#13283](https://github.com/mastra-ai/mastra/pull/13283))
  - Updated dependency [`semver@^7.7.4` â†—ï¸Ž](https://www.npmjs.com/package/semver/v/7.7.4) (from `^7.7.2`, in `dependencies`)

- dependencies updates: ([#13771](https://github.com/mastra-ai/mastra/pull/13771))
  - Updated dependency [`@uiw/codemirror-theme-dracula@^4.25.5` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/codemirror-theme-dracula/v/4.25.5) (from `^4.25.4`, in `dependencies`)

- dependencies updates: ([#13847](https://github.com/mastra-ai/mastra/pull/13847))
  - Updated dependency [`@codemirror/autocomplete@^6.20.1` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/autocomplete/v/6.20.1) (from `^6.20.0`, in `dependencies`)
  - Updated dependency [`@codemirror/lang-javascript@^6.2.5` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/lang-javascript/v/6.2.5) (from `^6.2.4`, in `dependencies`)
  - Updated dependency [`@codemirror/view@^6.39.16` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/view/v/6.39.16) (from `^6.39.15`, in `dependencies`)
...
- Redesigned experiment detail page with tabbed layout, score integration, and modular column panels. ([#13663](https://github.com/mastra-ai/mastra/pull/13663))
  ...
  **Score detail panel**
  ...
  - Score and trace panels are mutually exclusive â€?opening one closes the other
  ...
  **Component cleanup**
  - Removed `ListAndDetails` DS component â€?replaced by `Column` sub-components (`Column.Toolbar`, `Column.Content`)
...
- Improved the score dialog to show "N/A" with an explanation instead of "null" for empty scorer fields. Code-based scorers show "N/A â€?code-based scorer does not use prompts" and LLM scorers with unconfigured steps show "N/A â€?step not configured". Detection uses the `hasJudge` metadata flag with a heuristic fallback for older data. ([#13386](https://github.com/mastra-ai/mastra/pull/13386))
...
- dependencies updates: ([#13284](https://github.com/mastra-ai/mastra/pull/13284))
  - Updated dependency [`sonner@^2.0.7` â†—ï¸Ž](https://www.npmjs.com/package/sonner/v/2.0.7) (from `^2.0.5`, in `dependencies`)

- dependencies updates: ([#13300](https://github.com/mastra-ai/mastra/pull/13300))
  - Updated dependency [`superjson@^2.2.6` â†—ï¸Ž](https://www.npmjs.com/package/superjson/v/2.2.6) (from `^2.2.2`, in `dependencies`)
...
- dependencies updates: ([#12921](https://github.com/mastra-ai/mastra/pull/12921))
  - Updated dependency [`@assistant-ui/react@^0.12.10` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react/v/0.12.10) (from `^0.12.3`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-markdown@^0.12.3` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-markdown/v/0.12.3) (from `^0.12.1`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-syntax-highlighter@^0.12.3` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-syntax-highlighter/v/0.12.3) (from `^0.12.1`, in `dependencies`)

- dependencies updates: ([#13100](https://github.com/mastra-ai/mastra/pull/13100))
  - Updated dependency [`@codemirror/view@^6.39.14` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/view/v/6.39.14) (from `^6.39.13`, in `dependencies`)
...
- Added a dedicated Integration Tools section to the agent CMS tools page. Providers are shown as cards â€?clicking one opens a side dialog with toolkit navigation on the left and a searchable, multi-select tool list on the right. Selected tools persist to the agent form. Regular tools now appear in a separate "Available Tools" section. ([#13048](https://github.com/mastra-ai/mastra/pull/13048))
...
- dependencies updates: ([#12949](https://github.com/mastra-ai/mastra/pull/12949))
  - Updated dependency [`@codemirror/view@^6.39.13` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/view/v/6.39.13) (from `^6.39.12`, in `dependencies`)
...
- dependencies updates: ([#11750](https://github.com/mastra-ai/mastra/pull/11750))
  - Updated dependency [`@assistant-ui/react@^0.12.3` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react/v/0.12.3) (from `^0.11.47`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-markdown@^0.12.1` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-markdown/v/0.12.1) (from `^0.11.6`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-syntax-highlighter@^0.12.1` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-syntax-highlighter/v/0.12.1) (from `^0.11.6`, in `dependencies`)

- dependencies updates: ([#11829](https://github.com/mastra-ai/mastra/pull/11829))
  - Updated dependency [`@codemirror/autocomplete@^6.20.0` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/autocomplete/v/6.20.0) (from `^6.18.0`, in `dependencies`)
  - Updated dependency [`@codemirror/lang-javascript@^6.2.4` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/lang-javascript/v/6.2.4) (from `^6.2.2`, in `dependencies`)
  - Updated dependency [`@codemirror/state@^6.5.4` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/state/v/6.5.4) (from `^6.5.2`, in `dependencies`)
  - Updated dependency [`@codemirror/view@^6.39.12` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/view/v/6.39.12) (from `^6.38.6`, in `dependencies`)

- dependencies updates: ([#11830](https://github.com/mastra-ai/mastra/pull/11830))
  - Updated dependency [`@uiw/codemirror-theme-dracula@^4.25.4` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/codemirror-theme-dracula/v/4.25.4) (from `^4.23.14`, in `dependencies`)

- dependencies updates: ([#11843](https://github.com/mastra-ai/mastra/pull/11843))
  - Updated dependency [`@uiw/react-codemirror@^4.25.4` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/react-codemirror/v/4.25.4) (from `^4.23.14`, in `dependencies`)

- dependencies updates: ([#12782](https://github.com/mastra-ai/mastra/pull/12782))
  - Updated dependency [`@uiw/codemirror-theme-github@^4.25.4` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/codemirror-theme-github/v/4.25.4) (from `^4.25.3`, in `dependencies`)
...
- **Async buffering for observational memory is now enabled by default.** Observations are pre-computed in the background as conversations grow â€?when the context window fills up, buffered observations activate instantly with no blocking LLM call. This keeps agents responsive during long conversations. ([#12891](https://github.com/mastra-ai/mastra/pull/12891))

  **Default settings:**
  - `observation.bufferTokens: 0.2` â€?buffer every 20% of `messageTokens` (~6k tokens with the default 30k threshold)
  - `observation.bufferActivation: 0.8` â€?on activation, retain 20% of the message window
  - `reflection.bufferActivation: 0.5` â€?start background reflection at 50% of the observation threshold
...
- **Removed `storage.getMessages()`** ([#9695](https://github.com/mastra-ai/mastra/pull/9695))
  ...
  **Renamed `client.getThreadMessages()` â†?`client.listThreadMessages()`**
...
- dependencies updates: ([#10057](https://github.com/mastra-ai/mastra/pull/10057))
  - Updated dependency [`@uiw/codemirror-theme-github@^4.25.3` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/codemirror-theme-github/v/4.25.3) (from `^4.25.2`, in `dependencies`)

- dependencies updates: ([#10085](https://github.com/mastra-ai/mastra/pull/10085))
  - Updated dependency [`@xyflow/react@^12.9.3` â†—ï¸Ž](https://www.npmjs.com/package/@xyflow/react/v/12.9.3) (from `^12.8.6`, in `dependencies`)

- dependencies updates: ([#11404](https://github.com/mastra-ai/mastra/pull/11404))
  - Updated dependency [`zustand@^5.0.9` â†—ï¸Ž](https://www.npmjs.com/package/zustand/v/5.0.9) (from `^5.0.8`, in `dependencies`)
...
- dependencies updates: ([#9800](https://github.com/mastra-ai/mastra/pull/9800))
  - Updated dependency [`@lezer/highlight@^1.2.3` â†—ï¸Ž](https://www.npmjs.com/package/@lezer/highlight/v/1.2.3) (from `^1.2.1`, in `dependencies`)

- dependencies updates: ([#9850](https://github.com/mastra-ai/mastra/pull/9850))
  - Updated dependency [`@dagrejs/dagre@^1.1.8` â†—ï¸Ž](https://www.npmjs.com/package/@dagrejs/dagre/v/1.1.8) (from `^1.1.5`, in `dependencies`)
...
- dependencies updates: ([#8685](https://github.com/mastra-ai/mastra/pull/8685))
  - Updated dependency [`zod@^4.1.12` â†—ï¸Ž](https://www.npmjs.com/package/zod/v/4.1.12) (from `^4.1.9`, in `dependencies`)
...
- dependencies updates: ([#8298](https://github.com/mastra-ai/mastra/pull/8298))
  - Updated dependency [`@xyflow/react@^12.8.6` â†—ï¸Ž](https://www.npmjs.com/package/@xyflow/react/v/12.8.6) (from `^12.8.5`, in `dependencies`)

- dependencies updates: ([#8333](https://github.com/mastra-ai/mastra/pull/8333))
  - Updated dependency [`motion@^12.23.22` â†—ï¸Ž](https://www.npmjs.com/package/motion/v/12.23.22) (from `^12.23.13`, in `dependencies`)
...
- dependencies updates: ([#7980](https://github.com/mastra-ai/mastra/pull/7980))
  - Updated dependency [`zod@^4.1.8` â†—ï¸Ž](https://www.npmjs.com/package/zod/v/4.1.8) (from `^4.1.5`, in `dependencies`)

- dependencies updates: ([#8019](https://github.com/mastra-ai/mastra/pull/8019))
  - Updated dependency [`motion@^12.23.13` â†—ï¸Ž](https://www.npmjs.com/package/motion/v/12.23.13) (from `^12.23.12`, in `dependencies`)

- dependencies updates: ([#8034](https://github.com/mastra-ai/mastra/pull/8034))
  - Updated dependency [`zod@^4.1.9` â†—ï¸Ž](https://www.npmjs.com/package/zod/v/4.1.9) (from `^4.1.8`, in `dependencies`)

- dependencies updates: ([#8050](https://github.com/mastra-ai/mastra/pull/8050))
  - Updated dependency [`@xyflow/react@^12.8.5` â†—ï¸Ž](https://www.npmjs.com/package/@xyflow/react/v/12.8.5) (from `^12.8.4`, in `dependencies`)
...
- dependencies updates: ([#7802](https://github.com/mastra-ai/mastra/pull/7802))
  - Updated dependency [`react-syntax-highlighter@^15.6.6` â†—ï¸Ž](https://www.npmjs.com/package/react-syntax-highlighter/v/15.6.6) (from `^15.6.1`, in `dependencies`)

- dependencies updates: ([#7868](https://github.com/mastra-ai/mastra/pull/7868))
  - Updated dependency [`swr@^2.3.6` â†—ï¸Ž](https://www.npmjs.com/package/swr/v/2.3.6) (from `^2.3.4`, in `dependencies`)

- dependencies updates: ([#7908](https://github.com/mastra-ai/mastra/pull/7908))
  - Updated dependency [`use-debounce@^10.0.6` â†—ï¸Ž](https://www.npmjs.com/package/use-debounce/v/10.0.6) (from `^10.0.5`, in `dependencies`)

- dependencies updates: ([#7912](https://github.com/mastra-ai/mastra/pull/7912))
  - Updated dependency [`zustand@^5.0.8` â†—ï¸Ž](https://www.npmjs.com/package/zustand/v/5.0.8) (from `^5.0.7`, in `dependencies`)
...
- ab48c97: dependencies updates:
  - Updated dependency [`zod@^4.1.5` â†—ï¸Ž](https://www.npmjs.com/package/zod/v/4.1.5) (from `^4.0.15`, in `dependencies`)
- dbdc91f: dependencies updates:
  - Updated dependency [`@assistant-ui/react@^0.10.44` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react/v/0.10.44) (from `^0.7.91`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-markdown@^0.10.9` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-markdown/v/0.10.9) (from `^0.7.21`, in `dependencies`)
  - Updated dependency [`@assistant-ui/react-syntax-highlighter@^0.10.10` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-syntax-highlighter/v/0.10.10) (from `^0.7.10`, in `dependencies`)
  - Added dependency [`@assistant-ui/react-ui@^0.1.8` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react-ui/v/0.1.8) (to `dependencies`)
- 0fe56ce: dependencies updates:
  - Updated dependency [`@xyflow/react@^12.8.4` â†—ï¸Ž](https://www.npmjs.com/package/@xyflow/react/v/12.8.4) (from `^12.8.2`, in `dependencies`)
- 572acd0: dependencies updates:
  - Updated dependency [`@assistant-ui/react@^0.10.45` â†—ï¸Ž](https://www.npmjs.com/package/@assistant-ui/react/v/0.10.45) (from `^0.10.44`, in `dependencies`)
...
- 0d32203: dependencies updates:
  - Updated dependency [`zustand@^5.0.7` â†—ï¸Ž](https://www.npmjs.com/package/zustand/v/5.0.7) (from `^5.0.6`, in `dependencies`)
...
- 0ebfe8a: dependencies updates:
  - Updated dependency [`motion@^12.23.12` â†—ï¸Ž](https://www.npmjs.com/package/motion/v/12.23.12) (from `^12.23.9`, in `dependencies`)
...
- d8e8349: dependencies updates:
  - Updated dependency [`@xyflow/react@^12.8.2` â†—ï¸Ž](https://www.npmjs.com/package/@xyflow/react/v/12.8.2) (from `^12.8.1`, in `dependencies`)
- c8924b6: dependencies updates:
  - Updated dependency [`motion@^12.23.9` â†—ï¸Ž](https://www.npmjs.com/package/motion/v/12.23.9) (from `^12.23.0`, in `dependencies`)
...
- f873f3a: dependencies updates:
  - Updated dependency [`zustand@^5.0.6` â†—ï¸Ž](https://www.npmjs.com/package/zustand/v/5.0.6) (from `^5.0.5`, in `dependencies`)
...
- 5130bcb: dependencies updates:
  - Updated dependency [`swr@^2.3.4` â†—ï¸Ž](https://www.npmjs.com/package/swr/v/2.3.4) (from `^2.3.3`, in `dependencies`)
- 984887a: dependencies updates:
  - Updated dependency [`prettier@^3.6.2` â†—ï¸Ž](https://www.npmjs.com/package/prettier/v/3.6.2) (from `^3.5.3`, in `dependencies`)
...
- 7fb0909: dependencies updates:
  - Updated dependency [`@dagrejs/dagre@^1.1.5` â†—ï¸Ž](https://www.npmjs.com/package/@dagrejs/dagre/v/1.1.5) (from `^1.1.4`, in `dependencies`)
- 05ba777: dependencies updates:
  - Updated dependency [`@xyflow/react@^12.8.1` â†—ï¸Ž](https://www.npmjs.com/package/@xyflow/react/v/12.8.1) (from `^12.6.4`, in `dependencies`)
- 7ccbf43: dependencies updates:
  - Updated dependency [`motion@^12.23.0` â†—ï¸Ž](https://www.npmjs.com/package/motion/v/12.23.0) (from `^12.16.0`, in `dependencies`)
- 4b4b339: dependencies updates:
  - Updated dependency [`@uiw/react-codemirror@^4.23.14` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/react-codemirror/v/4.23.14) (from `^4.23.13`, in `dependencies`)
...
- 8e1b6e9: dependencies updates:
  - Updated dependency [`zod@^3.25.67` â†—ï¸Ž](https://www.npmjs.com/package/zod/v/3.25.67) (from `^3.25.57`, in `dependencies`)
- d569c16: dependencies updates:
  - Updated dependency [`react-code-block@1.1.3` â†—ï¸Ž](https://www.npmjs.com/package/react-code-block/v/1.1.3) (from `1.1.1`, in `dependencies`)
- b9f5599: dependencies updates:
  - Updated dependency [`@codemirror/lang-json@^6.0.2` â†—ï¸Ž](https://www.npmjs.com/package/@codemirror/lang-json/v/6.0.2) (from `^6.0.1`, in `dependencies`)
...
- 63f6b7d: dependencies updates:
  - Updated dependency [`@ai-sdk/ui-utils@^1.2.11` â†—ï¸Ž](https://www.npmjs.com/package/@ai-sdk/ui-utils/v/1.2.11) (from `^1.1.19`, in `dependencies`)
  - Updated dependency [`@autoform/core@^2.2.0` â†—ï¸Ž](https://www.npmjs.com/package/@autoform/core/v/2.2.0) (from `^2.1.0`, in `dependencies`)
  - Updated dependency [`@autoform/react@^3.1.0` â†—ï¸Ž](https://www.npmjs.com/package/@autoform/react/v/3.1.0) (from `^3.0.0`, in `dependencies`)
  - Updated dependency [`@autoform/zod@^2.2.0` â†—ï¸Ž](https://www.npmjs.com/package/@autoform/zod/v/2.2.0) (from `^2.1.0`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-avatar@^1.1.10` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-avatar/v/1.1.10) (from `^1.1.3`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-checkbox@^1.3.2` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-checkbox/v/1.3.2) (from `^1.1.4`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-collapsible@^1.1.11` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-collapsible/v/1.1.11) (from `^1.1.3`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-dialog@^1.1.14` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-dialog/v/1.1.14) (from `^1.1.6`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-label@^2.1.7` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-label/v/2.1.7) (from `^2.1.2`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-popover@^1.1.14` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-popover/v/1.1.14) (from `^1.1.6`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-scroll-area@^1.2.9` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-scroll-area/v/1.2.9) (from `^1.2.3`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-select@^2.2.5` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-select/v/2.2.5) (from `^2.1.6`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-slider@^1.3.5` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-slider/v/1.3.5) (from `^1.2.3`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-slot@^1.2.3` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-slot/v/1.2.3) (from `^1.1.2`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-switch@^1.2.5` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-switch/v/1.2.5) (from `^1.1.3`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-tabs@^1.1.12` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-tabs/v/1.1.12) (from `^1.1.2`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-toggle@^1.1.9` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-toggle/v/1.1.9) (from `^1.1.2`, in `dependencies`)
  - Updated dependency [`@radix-ui/react-tooltip@^1.2.7` â†—ï¸Ž](https://www.npmjs.com/package/@radix-ui/react-tooltip/v/1.2.7) (from `^1.1.8`, in `dependencies`)
  - Updated dependency [`@tanstack/react-table@^8.21.3` â†—ï¸Ž](https://www.npmjs.com/package/@tanstack/react-table/v/8.21.3) (from `^8.21.2`, in `dependencies`)
  - Updated dependency [`@uiw/codemirror-theme-dracula@^4.23.13` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/codemirror-theme-dracula/v/4.23.13) (from `^4.23.10`, in `dependencies`)
  - Updated dependency [`@uiw/react-codemirror@^4.23.13` â†—ï¸Ž](https://www.npmjs.com/package/@uiw/react-codemirror/v/4.23.13) (from `^4.23.8`, in `dependencies`)
  - Updated dependency [`@xyflow/react@^12.6.4` â†—ï¸Ž](https://www.npmjs.com/package/@xyflow/react/v/12.6.4) (from `^12.3.6`, in `dependencies`)
  - Updated dependency [`cmdk@^1.1.1` â†—ï¸Ž](https://www.npmjs.com/package/cmdk/v/1.1.1) (from `^1.0.0`, in `dependencies`)
  - Updated dependency [`json-schema-to-zod@^2.6.1` â†—ï¸Ž](https://www.npmjs.com/package/json-schema-to-zod/v/2.6.1) (from `^2.5.0`, in `dependencies`)
  - Updated dependency [`motion@^12.16.0` â†—ï¸Ž](https://www.npmjs.com/package/motion/v/12.16.0) (from `^12.4.2`, in `dependencies`)
  - Updated dependency [`prism-react-renderer@^2.4.1` â†—ï¸Ž](https://www.npmjs.com/package/prism-react-renderer/v/2.4.1) (from `^2.4.0`, in `dependencies`)
  - Updated dependency [`react-hook-form@^7.57.0` â†—ï¸Ž](https://www.npmjs.com/package/react-hook-form/v/7.57.0) (from `^7.54.2`, in `dependencies`)
  - Updated dependency [`react-resizable-panels@^2.1.9` â†—ï¸Ž](https://www.npmjs.com/package/react-resizable-panels/v/2.1.9) (from `^2.1.7`, in `dependencies`)
  - Updated dependency [`remark-rehype@^11.1.2` â†—ï¸Ž](https://www.npmjs.com/package/remark-rehype/v/11.1.2) (from `^11.1.1`, in `dependencies`)
  - Updated dependency [`remeda@^2.23.0` â†—ï¸Ž](https://www.npmjs.com/package/remeda/v/2.23.0) (from `^2.21.1`, in `dependencies`)
  - Updated dependency [`sonner@^2.0.5` â†—ï¸Ž](https://www.npmjs.com/package/sonner/v/2.0.5) (from `^2.0.1`, in `dependencies`)
  - Updated dependency [`use-debounce@^10.0.5` â†—ï¸Ž](https://www.npmjs.com/package/use-debounce/v/10.0.5) (from `^10.0.4`, in `dependencies`)
  - Updated dependency [`zod@^3.25.57` â†—ï¸Ž](https://www.npmjs.com/package/zod/v/3.25.57) (from `^3.25.56`, in `dependencies`)
  - Updated dependency [`zustand@^5.0.5` â†—ï¸Ž](https://www.npmjs.com/package/zustand/v/5.0.5) (from `^5.0.3`, in `dependencies`)
...
- 1ccccff: dependencies updates:
  - Updated dependency [`zod@^3.25.56` â†—ï¸Ž](https://www.npmjs.com/package/zod/v/3.25.56) (from `^3.24.3`, in `dependencies`)
- 1ccccff: dependencies updates:
  - Updated dependency [`zod@^3.25.56` â†—ï¸Ž](https://www.npmjs.com/package/zod/v/3.25.56) (from `^3.24.3`, in `dependencies`)
...
- 2cf3b8f: dependencies updates:
  - Updated dependency [`zod@^3.24.3` â†—ï¸Ž](https://www.npmjs.com/package/zod/v/3.24.3) (from `^3.24.2`, in `dependencies`)
