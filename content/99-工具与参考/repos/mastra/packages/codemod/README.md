---
title: Mastra Codemods
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/codemod/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
| `v1/agent-generate-stream-v-next`    | Renames `agent.generateVNext()` â†?`agent.generate()` and `agent.streamVNext()` â†?`agent.stream()`                  |
| `v1/agent-processor-methods`         | Renames `agent.getInputProcessors()` â†?`agent.listInputProcessors()` and similar output processor methods          |
| `v1/agent-property-access`           | Transforms agent property access to method calls: `agent.llm` â†?`agent.getLLM()`                                   |
| `v1/agent-voice`                     | Moves agent voice methods to namespace: `agent.speak()` â†?`agent.voice.speak()`                                    |
...
| `v1/evals-run-experiment`            | Renames `runExperiment()` â†?`runEvals()` in imports and usages                                                     |
| `v1/evals-scorer-by-name`            | Renames `mastra.getScorerByName()` â†?`mastra.getScorerById()`                                                      |
...
| `v1/mcp-get-tools`                   | Renames `mcp.getTools()` â†?`mcp.listTools()`                                                                       |
| `v1/mcp-get-toolsets`                | Renames `mcp.getToolsets()` â†?`mcp.listToolsets()`                                                                 |
| `v1/memory-message-v2-type`          | Renames `MastraMessageV2` type â†?`MastraDBMessage` in imports and usages                                           |
| `v1/memory-query-to-recall`          | Renames `memory.query()` â†?`memory.recall()`                                                                       |
| `v1/memory-vector-search-param`      | Renames `vectorMessageSearch` parameter â†?`vectorSearchString` in `memory.recall()` calls                          |
...
| `v1/storage-get-messages-paginated`  | Renames `storage.getMessagesPaginated()` â†?`storage.listMessages()` and `offset`/`limit` â†?`page`/`perPage`        |
| `v1/storage-get-threads-by-resource` | Renames `storage.getThreadsByResourceId()` â†?`storage.listThreadsByResourceId()`                                   |
| `v1/storage-list-messages-by-id`     | Renames `storage.getMessagesById()` â†?`storage.listMessagesById()`                                                 |
| `v1/storage-list-workflow-runs`      | Renames `storage.getWorkflowRuns()` â†?`storage.listWorkflowRuns()`                                                 |
| `v1/storage-postgres-schema-name`    | Renames `schema` property â†?`schemaName` in PostgresStore constructor                                              |
| `v1/vector-pg-constructor`           | Converts `new PgVector(connectionString)` to `new PgVector({ connectionString })`                                  |
| `v1/voice-property-names`            | Renames voice property names in Agent configuration: `speakProvider` â†?`output`                                    |
| `v1/workflow-create-run-async`       | Renames `workflow.createRunAsync()` â†?`workflow.createRun()`                                                       |
| `v1/workflow-list-runs`              | Renames `workflow.getWorkflowRuns()` â†?`workflow.listWorkflowRuns()`                                               |
| `v1/workflow-run-count`              | Renames `context.runCount` â†?`context.retryCount` in step execution functions                                      |
