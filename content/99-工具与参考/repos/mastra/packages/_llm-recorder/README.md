---
title: @internal/llm-recorder
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/_llm-recorder/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
LLM response recording and replay for tests. Works like test snapshots â€?auto-records on first run, replays deterministically thereafter.
...
Enable recording for all test files automatically â€?no changes to test files needed:
...
- `packages/memory/src/index.test.ts` â†?`memory-src-index`
- `stores/pg/src/storage.test.ts` â†?`pg-src-storage`
...
Works like Vitest snapshots â€?auto-records on first run, replays thereafter:
...
# Strict replay â€?fail if no recording exists
LLM_TEST_MODE=replay pnpm test
```

**Mode Selection Priority:**

1. `--update-recordings` flag or `UPDATE_RECORDINGS=true` â†?update (force re-record)
2. `LLM_TEST_MODE=live` â†?live (no recording)
3. `LLM_TEST_MODE=record` â†?record (legacy, same as update)
4. `LLM_TEST_MODE=replay` â†?replay (strict, fail if no recording)
5. `RECORD_LLM=true` â†?record (legacy)
6. Default â†?**auto** (replay if exists, record if not)
...
| Export                                 | Description                                                                  |
| -------------------------------------- | ---------------------------------------------------------------------------- |
| `useLLMRecording(name, options?)`      | Vitest helper â€?sets up `beforeAll`/`afterAll` hooks                         |
...
```
your-package/
â”œâ”€â”€ __recordings__/
â”?  â”œâ”€â”€ my-agent-tests.json
â”?  â””â”€â”€ a1b2c3d4-response.wav
â””â”€â”€ src/
    â””â”€â”€ tests/
```
...
| ------ | ------------------------------------ | ----------------------- |
| Auto   | <100ms (replay) or 5-30s (first run) | Default â€?just works    |
