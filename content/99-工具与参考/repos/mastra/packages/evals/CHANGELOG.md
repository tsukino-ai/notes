...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/evals/CHANGELOG.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- Fixed prebuilt LLM-judge scorers (faithfulness, answer relevancy, bias, hallucination, toxicity, â€? crashing with `TypeError: output.find is not a function` when used against workflows, inline tasks, or string targets. The shared input/output helpers and scorer run types now also accept `string`, `ModelMessage[]`, `{ prompt }` (workflow input), `{ text }` / `{ content }` (workflow / task output), and a single assistant message object â€?alongside the existing agent shape. ([#15805](https://github.com/mastra-ai/mastra/pull/15805))
...
- **Trajectory scorers**: Added scorers for evaluating agent and workflow execution paths. ([#14697](https://github.com/mastra-ai/mastra/pull/14697))
  - `createTrajectoryScorerCode` â€?unified scorer that evaluates accuracy, efficiency, blacklist violations, and tool failure patterns in a single pass. Supports per-item expectations from datasets with static defaults. Nested `ExpectedStep.children` configs allow recursive evaluation with different rules per hierarchy level.
  - `createTrajectoryAccuracyScorerCode` â€?deterministic accuracy scorer with strict, relaxed, and unordered ordering modes.
  - `createTrajectoryAccuracyScorerLLM` â€?LLM-based scorer for semantic trajectory evaluation.

  **Utility functions:**
  - `extractTrajectory` / `extractWorkflowTrajectory` â€?Convert agent runs and workflow executions into structured trajectories
  - `extractTrajectoryFromTrace` â€?Build hierarchical trajectories from observability trace spans, including nested agent/tool calls
  - `compareTrajectories` â€?Compare actual vs. expected trajectories with configurable ordering and data matching. Accepts `ExpectedStep[]` for simpler expected step definitions
  - `checkTrajectoryEfficiency` â€?Evaluate step counts, token usage, and duration against budgets
  - `checkTrajectoryBlacklist` â€?Detect forbidden tools or tool sequences
  - `analyzeToolFailures` â€?Detect retry patterns, fallbacks, and argument corrections

  **Example â€?unified scorer with defaults:**
...
- **Configurable weights**: Add `weights` option to `createTrajectoryScorerCode` for controlling how dimension scores are combined. Defaults to `{ accuracy: 0.4, efficiency: 0.3, toolFailures: 0.2, blacklist: 0.1 }`. ([#14740](https://github.com/mastra-ai/mastra/pull/14740))
  ...
  **Remove `compareStepData`**: The `compareStepData` option is removed from `compareTrajectories`, `TrajectoryExpectation`, and all scorers. Data fields are now auto-compared when present on expected steps â€?if you specify `toolArgs` on an `ExpectedStep`, it will be compared against the actual step. If you omit it, only name and stepType are matched.
...
- dependencies updates: ([#12998](https://github.com/mastra-ai/mastra/pull/12998))
  - Updated dependency [`compromise@^14.14.5` â†—ï¸Ž](https://www.npmjs.com/package/compromise/v/14.14.5) (from `^14.14.4`, in `dependencies`)
...
- dependencies updates: ([#8007](https://github.com/mastra-ai/mastra/pull/8007))
  - Updated dependency [`fs-extra@^11.3.2` â†—ï¸Ž](https://www.npmjs.com/package/fs-extra/v/11.3.2) (from `^11.3.1`, in `dependencies`)
...
- dependencies updates: ([#7544](https://github.com/mastra-ai/mastra/pull/7544))
  - Updated dependency [`fs-extra@^11.3.1` â†—ï¸Ž](https://www.npmjs.com/package/fs-extra/v/11.3.1) (from `^11.3.0`, in `dependencies`)
