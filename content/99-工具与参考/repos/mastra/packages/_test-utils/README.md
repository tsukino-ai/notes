---
title: @internal/test-utils
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/_test-utils/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
Returns a self-contained instance â€?no global state. You control the lifecycle.
...
- `name` â€?Explicit recording name (auto-derived from test file path if omitted)
- `recordingsDir` â€?Directory for recording files (default: `__recordings__` in cwd)
- `forceRecord` â€?Force re-record even if recording exists
- `replayWithTiming` â€?Replay with original chunk timing
- `maxChunkDelay` â€?Max delay between chunks in replay, ms (default: 10)
- `transformRequest` â€?Transform requests before hashing
- `extraHosts` â€?Additional API hosts to intercept (merged with auto-detected ones)
- `debug` â€?Enable verbose debug logging
