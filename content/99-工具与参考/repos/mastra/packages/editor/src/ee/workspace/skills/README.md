---
title: Agent Builder â€?Authoring Skills
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/editor/src/ee/workspace/skills/README.md
tags:
  - agent-framework
  - typescript
  - mastra
...
# Agent Builder â€?Authoring Skills
...
- `skill_search` â€?find a skill by name/description.
- `skill` â€?activate a skill; returns the full `SKILL.md` content as a tool result.
- `skill_read` â€?read a specific file inside a skill directory (references, scripts, assets).
...
1. **Trigger / input** â€?what starts a run: a user message, schedule, webhook, file, row, ticket, document, or event.
2. **Owned outcome** â€?the one concrete result the agent is responsible for finishing.
3. **Available capabilities** â€?only tools, data sources, workflows, agents, or stored skills actually attached or available.
4. **Missing-capability fallback** â€?what the agent does when a required integration, credential, workspace, permission, source, or input is absent.
5. **Done criteria** â€?how the agent proves the job is complete: tool confirmation, read-back, tests, citations, delivery receipt, or an explicit not-run reason.
6. **Final response format** â€?the exact receipt, report, draft, diff summary, confirmation, or escalation note the user receives.
...
1. **YAML frontmatter** â€?`name` (must match directory name; lowercase + hyphens), `description` (1024 chars max; includes the user-facing trigger words so `skill_search` ranks it correctly).
2. **When to use** â€?trigger criteria.
3. **Agent identity template** â€?name/description patterns.
4. **Domain-specific policy** â€?mode selection, safety boundaries, freshness, action boundary, or missing-input rules.
5. **System prompt template** â€?a fully-fleshed system prompt with placeholders the builder must instantiate.
6. **Required behavioral rules** â€?decisiveness, output format, **completion criteria (the most important section)**, refusals.
7. **Capabilities to prefer** â€?which tools/skills/sub-agents to attach for this archetype.
8. **Anti-patterns** â€?concrete bad-prompt shapes to reject.
9. **Worked example** â€?a full user request â†?produced agent example.
...
5. Include explicit **completion criteria** in the produced system prompt template. This is the single most important rule â€?without it, agents do not finish their work.
...
7. No code changes needed â€?the workspace loads `*/SKILL.md` files at agent-build time via the static `skills: ['skills']` path in `agent-builder-agent.ts`.
