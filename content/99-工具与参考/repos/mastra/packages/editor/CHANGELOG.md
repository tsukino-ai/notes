...
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/packages/editor/CHANGELOG.md
tags:
  - agent-framework
  - typescript
  - mastra
...
- Added optional constructor argument support to `createBuilderAgent()` so callers can override agent defaults while the canonical `id`, `name`, and `description` are preserved. ([#17109](https://github.com/mastra-ai/mastra/pull/17109))
  ...
  ```ts
  // Before â€?could only use the built-in defaults
  const builder = createBuilderAgent();

  // After â€?pass overrides while keeping the canonical identity
  const builder = createBuilderAgent({
    model: openai('gpt-4o'),
...
- Ship `EditorAgentBuilder` and Agent Builder runtime through the `@mastra/editor/ee` subpath. ([#16948](https://github.com/mastra-ai/mastra/pull/16948))
  ...
  This is plumbing â€?no UI consumer ships in this release.
...
- `EditorWorkspaceNamespace` can now snapshot a live `Workspace` for persistence â€?the reverse of `hydrateSnapshotToWorkspace()`: ([#16673](https://github.com/mastra-ai/mastra/pull/16673))
...
- Added an `editor.favorites` namespace so direct (non-HTTP) callers can favorite, unfavorite, and query favorited stored agents/skills through the editor instance. ([#16749](https://github.com/mastra-ai/mastra/pull/16749))
  ...
  The namespace performs the storage mutation only â€?visibility and ownership enforcement still belong to the caller (the HTTP route handlers in `@mastra/server` already do this).
...
- dependencies updates: ([#14052](https://github.com/mastra-ai/mastra/pull/14052))
  - Updated dependency [`@composio/mastra@^0.6.4` â†—ï¸Ž](https://www.npmjs.com/package/@composio/mastra/v/0.6.4) (from `^0.6.3`, in `dependencies`)
...
- dependencies updates: ([#14046](https://github.com/mastra-ai/mastra/pull/14046))
  - Updated dependency [`@composio/core@^0.6.4` â†—ï¸Ž](https://www.npmjs.com/package/@composio/core/v/0.6.4) (from `^0.6.3`, in `dependencies`)
...
- Added `FilesystemStore`, a file-based storage adapter for editor domains. Stores agent configurations, prompt blocks, scorer definitions, MCP clients, MCP servers, workspaces, and skills as JSON files in a local directory (default: `.mastra-storage/`). Only published snapshots are written to disk â€?version history is kept in memory. Use with `MastraCompositeStore`'s `editor` shorthand to enable Git-friendly editor configurations. ([#13727](https://github.com/mastra-ai/mastra/pull/13727))
  ...
  Added `applyStoredOverrides` to the editor agent namespace. When a stored configuration exists for a code-defined agent, the editor merges the stored **instructions** and **tools** on top of the code agent's values at runtime. Model, memory, workspace, and other code-defined fields are never overridden â€?they may contain SDK instances or dynamic functions that cannot be safely serialized. Original code-defined values are preserved via a WeakMap and restored if the stored override is deleted.
...
- Added MCP client management, integration tools resolution, and built-in Composio and Arcade AI tool providers. ([#12974](https://github.com/mastra-ai/mastra/pull/12974))
  ...
  **Built-in Tool Providers**
  - `@mastra/editor/composio` â€?Composio tool provider with toolkit/tool discovery and execution via `@composio/core` and `@composio/mastra` SDKs
  - `@mastra/editor/arcade` â€?Arcade AI tool provider with a pre-seeded catalog of 93 toolkits, tool discovery, and execution via `@arcadeai/arcadejs` SDK

  Each provider is a separate entry point â€?importing `@mastra/editor` alone does not load any provider SDK code.
...
- Added dynamic instructions for stored agents. Agent instructions can now be composed from reusable prompt blocks with conditional rules and variable interpolation, enabling a prompt-CMS-like editing experience. ([#12861](https://github.com/mastra-ai/mastra/pull/12861))
  ...
  **Instruction blocks** can be mixed in an agent's instructions array:
  - `text` â€?static text with `{{variable}}` interpolation
  - `prompt_block_ref` â€?reference to a versioned prompt block stored in the database
  - `prompt_block` â€?inline prompt block with optional conditional rules
  ...
  **Creating a prompt block and using it in a stored agent:**
  ...
  Prompt blocks are versioned â€?updating a block's content takes effect immediately for all agents referencing it, with no cache clearing required.
