---
title: Architecture Reference: Google Gemini Live Voice API Package
author: mastra-ai
date: 2026-05-28
source: https://github.com/mastra-ai/mastra/blob/main/voice/google-gemini-live-api/architecture-llm.md
tags:
  - agent-framework
  - typescript
  - mastra
...
```
src/
â”œâ”€â”€ index.ts                    # Main GeminiLiveVoice class
â”œâ”€â”€ types.ts                    # Type definitions
â”œâ”€â”€ utils/errors.ts            # Error handling
â””â”€â”€ managers/
    â”œâ”€â”€ AudioStreamManager.ts  # Audio processing
    â”œâ”€â”€ AuthManager.ts         # Authentication
    â”œâ”€â”€ ConnectionManager.ts   # WebSocket management
    â”œâ”€â”€ ContextManager.ts      # Conversation context
    â”œâ”€â”€ EventManager.ts        # Event system
    â””â”€â”€ SessionManager.ts      # Session lifecycle
```
...
**States**: `disconnected` â†?`connecting` â†?`connected` â†?`disconnected`
...
- `processAudioChunk(chunk)`: Convert Buffer â†?Int16Array â†?Base64
...
- `ZodString` â†?`{ type: 'string' }`
- `ZodNumber` â†?`{ type: 'number' }`
- `ZodBoolean` â†?`{ type: 'boolean' }`
- `ZodObject` â†?`{ type: 'object', properties: {...}, required: [...] }`
- `ZodEnum` â†?`{ type: 'string', enum: [...] }`
...
```typescript
// Input: Buffer â†?Int16Array â†?Base64 â†?WebSocket
...
// Output: Base64 â†?Int16Array â†?Buffer â†?Stream
