...
3. **翻译与网页发布（如果是英文原文，用户未明确拒绝时执行）**
   ...
   e. **生成中英段落对照文件（网页版，关键！）**：
      - **分块翻译**：长文章需拆分为 ~3,500-8,000 字符的 chunk，每个 chunk 启动一个翻译 subagent，避免 300s 超时
      - **翻译格式**：保留原文**段落结构**，每个英文段落下方紧跟中文译文
      - **输出格式**（Quartz callout 风格）：
        ```markdown
        [英文段落]
        ...
        ---
        ```
      - **合并**：所有 chunk 翻译完成后，按顺序合并为单一文件
      - **保存到**：`content/99-工具与参考/ref-articles/<slug>-bilingual.md`
      - **添加 frontmatter**（title, author, date, source）
   f. **在笔记中引用**：在笔记头部添加 `> [📖 中英段落对照阅读](../99-工具与参考/ref-articles/<slug>-bilingual.md)`

> **为什么用"段落对照"而非分开两个文件？**
> 1. **保留段落完整性**：以段落为单位对照，避免逐句拆分破坏上下文和论证流
> 2. **沉浸式学习体验**：一段英文紧跟一段中文，无需切换页面或滚动对齐
> 3. **网页友好**：单页加载，callout 样式在 Quartz 中渲染清晰，移动端阅读体验好
> 4. **可维护性**：段落结构稳定，后续原文更新时更容易定位对应译文
>
> **为什么放在 `99-工具与参考/ref-articles/`？**
> - 不 clutter 主学习目录（10-70），保持笔记目录整洁
> - 统一存放所有外文原文的对照阅读，便于管理
> - 通过笔记中的相对链接一键跳转
