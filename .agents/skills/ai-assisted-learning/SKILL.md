...
3. **翻译与网页发布（如果是英文原文，用户未明确拒绝时执行）**
   ...
   e. **生成中英逐句对照文件（网页版，关键！）**：
      - 启动 subagent，读取原文并按**句子**拆分（句号/问号/感叹号），逐句翻译
      - 输出格式：`[英文句子]\n> [中文译文]\n\n`（沉浸式翻译风格，上下对照）
      - 保存到 `content/99-工具与参考/ref-articles/<slug>-bilingual.md`
      - 添加 frontmatter（title, author, date, source）
   f. **在笔记中引用**：在笔记头部添加 `> [📖 中英逐句对照阅读](../99-工具与参考/ref-articles/<slug>-bilingual.md)`

> **为什么用"逐句对照"而非分开两个文件？**
> 1. **沉浸式学习体验**：一句英文紧跟一句中文，无需切换页面或滚动对齐
> 2. **精准对照**：避免段落拆分不一致导致的错位
> 3. **网页友好**：单页加载，移动端阅读体验更好
>
